...
of female survey respondents. What factors could explain this lower ratio?
...
`r round(mean(filter(persons, gender == "F")$as_author, na.rm = T), 2)` compared to mens'
`r round(mean(filter(persons, gender == "M")$as_author, na.rm = T), 2)`.
But this gap is not large enough to explain the 2--3x difference relative to the rest of CS, and is not unique to Systems anyway [@elsevier17:gender].
...
The same ratio also shows up when we limit our observations to lead authors only, where gender may be more prominent. And lastly,
...
even when limiting to lead authors or single-blind reviews.
 This grade similarity extends to most other categories, except for originality
...
In addition to these two grade differences, women also reported reviewers as more understanding, helpful, and fair than men did
...
On the other hand, papers authored by women show more prior submissions on average: 
...
which are linked to better author evaluations of the reviews.
