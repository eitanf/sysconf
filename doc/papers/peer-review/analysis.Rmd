...
The very low representation of women among authors, even compared to the rest of CS, is characteristic of the field of systems, and not just of our data set [@anon20:gender]. We also found no evidence that women receive lower overall acceptance scores than men in their peer reviews
...
It does appear, however, that women have to work harder for their papers to be published, as has been observed in other fields [@hengel19:publishing]. On average, a female author had her paper previously submitted to
`r round(mean(filter(survey_with_confs, mapped_gender == "F")$prior_subs, na.rm = T), 2)`
conferences, compared to
`r round(mean(filter(survey_with_confs, mapped_gender == "M")$prior_subs, na.rm = T), 2)`
for males
(`r report_t(t.test(filter(survey_with_confs, mapped_gender == "F")$prior_subs, filter(survey_with_confs, mapped_gender == "M")$prior_subs))`).
This effort gap could be related to the well-known gender publication gap [@elsevier17:gender]. Surprisingly, the gap disappears when limiting the data to single-blind conferences
(`r report_t(t.test(filter(survey_with_confs, mapped_gender == "F", double_blind == F)$prior_subs, filter(survey_with_confs, mapped_gender == "M", double_blind == F)$prior_subs))`),
although we have no information on whether the previous submissions were single- or double-blind reviewed. We address this difference in Sec. \@ref(subsec:double-blind).

From a scoring point of view, women appear to get similar overall acceptance grades to men
(`r report_t(t.test(filter(survey_with_confs, mapped_gender == "F")$gr_overall, filter(survey_with_confs, mapped_gender == "M")$gr_overall))`),
Even when correcting for lead authorship or double-blind policies. This similarity holds for most other review grades, except for originality
...
In both cases, women scored significantly higher than men.
Even from a subjective point of view, women tend to find reviews more
understanding
(`r report_chi(chisq.test(table(survey_with_confs$gender, survey_with_confs$understanding)), 2)`),
more helpful,
(`r report_chi(chisq.test(table(survey_with_confs$gender, survey_with_confs$helpfulness)), 2)`),
and to a lesser degree, more fair
(`r report_chi(chisq.test(table(survey_with_confs$gender, survey_with_confs$fairness)), 2)`).
It is conceivable that these higher evaluations are related to the higher effort as well. In other words, papers that have gone through numerous rounds of reviews and improvement may be considered of higher quality.

## English Proficency

Another aspect of diversity in scientific publication is English-level proficiency. All of the papers, reviews, and communications in our conference set were conducted in English, but not all authors or reviewers are native English speakers. The effective use of language can affect both reviewers' understanding of the works, authors understanding of the reviews, and of course, subsequent reception of the papers [@crovella08:openness; @editage18:perspectives; @flowerdew99:writing]. How does the author experience vary based on this factor?

At least in our data, the answer appears to be "not much". From an objective grading perspective, all but one of the review grade categories have very similar distributions, both for teams with native English speakers and for teams with none, even in the presentation grade
...
(`r round(mean(filter(survey, any_english == F)$gr_originality, na.rm = T), 1)` vs.
`r round(mean(filter(survey, any_english == T)$gr_originality, na.rm = T), 1)`,
...
Many factors do not appear to be significantly interacting with double-blindness, such as: the gender, position, and experience (H-index) of the authors.
...
On the other hand, we noticed earlier that the experience of women appears different for single-blind and double-blind reviews. Double-blind reviewed conferences in our set also attract longer papers 
...
We doubt that double-blinding adequately explains all these phenomena [@godlee98:peer; @mcnutt90:blinding]. Instead, we propose a third explanatory variable: the reputation or prestige of a conference. This concept is too informal to measure, but we did collect two proxy conference metrics: the acceptance rate and H5-index (as measured by GS).
...
When correcting for conference quality, as measured by either metric, most of these associations fade or disappear. For example, the relationship between review length and double-blind reviews loses statistical significance when limiting ourselves to conferences with an acceptance rate of less then 0.2
...
Many conferences, including SIGCSE, do not employ a formal rebuttal process. And yet most
...
More generally, the experience level of authors who found rebuttals helpful, as measured by median previous publications from their GS profile, is about half that of those who did not
...
Response helpfulness appears to have also varied greatly by conference. When limiting ourselves to the eleven conferences that had a formal rebuttal process and at least ten unique authors responding to this question, three conferences had higher than average dissatisfaction rate with the rebuttal process: ASPLOS, ISC, and SOSP. Conversely, in four conferences, no more than 8% of respondents were dissatisfied with the rebuttals: MICRO, PPoPP, SC, and PLDI.

<!---
# Related Work

We are not aware of an other cross-sectional conference author survey in the systems field, and most author reviews in the literature or the Web cover either a specific conference [@beverly13:findings; @daume15:naacl; @papagiannaki07:author; @parno17:SPsurvey] or a wide journal authorship. For example, An older survey (N = 4037) found near-unanimous agreement among authors that their paper was improved as a result of peer review [@sense09:peer]. Similarly, a more recent survey of journal authors (N = 6903) found that a majority of authors found reviewer comments helpful, as we did [@editage18:perspectives]. But 70% of authors also found the peer-review process moderately to extremely difficult, which the study suggests could be linked to low English-language proficiency (our findings do not support this suggestion). Another journal author survey (N = 2128) found that about half of the papers submitted to one of four mega-journals had been previously rejected [@solomon14:survey]. A recent comprehensive report describes the modern peer review process and challenges in detail, and includes mean duration and number of reviews per paper, by field---again, only for journals [@publons18:peer].
-->
