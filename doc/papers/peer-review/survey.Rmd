...
We designed the survey to address the questions of bias and diversity in peer review. This section details its questions, responses, and initial interpretation.^[Most data, source code, and additional survey analysis, will be made available upon publication. Survey responses will remain confidential, but detailed statistics will be publicized as well.]
We sent our survey to all 5919 valid email addresses during the summer of 2018, and `r nrow(raw_survey)` authors responded. We asked a few demographic questions, as well as questions about the review process, repeated for up to three different papers from our data set.

Non-responses to a question were marked by NA and excluded from analyses. In a few cases, coauthors responded differently to the same question on the same paper, but these differences always tested statistically insignificant, so we picked an arbitrary response for the distribution analyses.
...
but still not significant enough to reject the null hypothesis of independence (`r report_chi(chisq.test(gov_table))`).
It is possible that students are over-represented in this survey, introducing bias [@parno17:SPsurvey]. On the other hand, it is also a useful data point for conferences with an educational mission, as the responses to the survey could help shape their policies to better address this target audience.
...
In addition to the English coauthor question, we asked several questions on each paper to understand its history. We asked these questions for each unique paper, up to three per author.
...
df <- survey %>%
  ...
  select(mor) %>%
  freq_and_prop()
knitr::kable(t(df), booktabs = T, align = "rrrrr") %>% kable_styling(position = "center", font_size = 8)
```

This response is not significantly associated with the team size (number of coauthors), or lead author's experience, gender, or sector. It is, however, correlated to the next question:
...
df <- tmp %>%
  group_by(paper_id) %>%
  summarize(prior = last(prior_subs)) %>%
  select(prior) %>%
  freq_and_prop()
knitr::kable(t(df), booktabs = T, align = "r") %>% kable_styling(position = "center", font_size = 8)
```

It is instructive to see that $\approx$ 40% papers are rejected on their first submission [@solomon14:survey; @wallach11:rebooting], and can take as many as 12 attempts to reach publication. 
Authors were also asked to type in the names of the venues they had previously submitted to. We note that many papers were accepted after previously being rejected from equivalent (or even the very same) conferences, which can be interpreted both positively and negatively. Some authors expressed frustration that the peer-review process can appear arbitrary [@anderson08:towards; @francois15:arbit; @vardi09:conferences; @vines11:cointoss]. On the other hand, other authors expressed that an effective peer review process provides feedback that improves the paper for the next submission. These mixed opinions, combined with the resubmission history of so many papers, suggests that selection bias in this survey played a lesser role in deciding authors' experience one way or another.

<!--
\hfill\break
__Please type in their names__

Because of the unstructured responses to this question, quantitative analysis is challenging. As a case study, we looked  only at computer architecture, for which four of the leading conferences are represented in our data set, and are of similar size and acceptance rates. we noted that most papers that had been previously rejected from these conferences, had been mostly submitted to one of these four as well. This table shows in each cell how many papers were rejected from that row's conference prior to acceptance in that column's conference.
...
were positive. This high percentage may be a little surprising, considering how many PC chairs and authors alike commented privately on how little difference rebuttals make [@daume15:naacl]. One cautionary reminder is that these statistics exclude rejected papers (not in the survey), and all the authors who didn't respond to this survey or to this question, which could bias the results. We take a deeper look at the effects of a rebuttal policy in Sec. \@ref(subsec:rebuttal-analysis).
...
df <- survey %>%
  ...
  select(reviews) %>%
  freq_and_prop()
knitr::kable(t(df), booktabs=TRUE, align=c("rrrrrrr")) %>% kable_styling(position = "center", latex_options = "scale_down")
```

The papers in our data set average more than four reviews per paper, far better than the typical 2+ reviews in an average CS journal [@publons18:peer, p. 21]). This could partially explain the attractiveness of conferences over journals, at least in systems. Authors were also asked to qualitatively approximate _how long each review_ was. it is encouraging to find over half of the responses showing one or more pages per review
...
df <- freq_and_prop(survey$understanding)
...
This result is consistent with other surveys of journal authors [@editage18:perspectives; @sense09:peer].
