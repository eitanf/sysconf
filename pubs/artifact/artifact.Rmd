...
The availability of experimental artifacts is not only crucial for reproducibility, but it also directly contributes to the transparency, reusability, and credibility of the work [@feitelson15:repeatability].
...
The hypothesis of this paper is therefore that because of its importance to reproducibility, artifacts sharing in systems significantly increases a paper's influence, as measured by citations.
...
The discussion section presents potential implications from these findings, as well as potential threats to the validity of this analysis.
...
Four conferences---SC, OOPSLA, PLDI, and SLE---archived their artifacts in the ACM's digital library.
...
We are also interested in measuring the post-hoc impact of each paper, as approximated by its number of citations.
...
(`r pct(sum(top_cites$max == 0), nrow(top_cites), 2)`%) have gathered no citations yet.
...
The only existing form of standardized artifact metadata was for the subset of conferences organized by the ACM with artifact badge initiatives.
...
`r sum(artifacts$badge)` artifact papers (`r pct(sum(artifacts$badge), nrow(artifacts))`%)
...
This ratio is somewhat similar to a comparable study that found that 73% of URLs in five open-access (OA) journals were live after five years [@saberi12:accessibility].
...
It should be noted, however, that many papers in PACT and MobiCom are hardware-related, where artifacts are often unfeasible.
...
Papers who received an "Artifact available" badge did fare a little worse than artifact papers who did not
...
One way to gauge the availability of the paper's text is to look it up on GS and see if an accessible version (eprint) is linked, which was recorded in our dataset.
...
Moreover, our dataset includes not only the availability of an eprint link on GS, but also the approximate duration since publication (in months) that it took GS to display this link, offering a quantitative measure of accessibility speed.
...
We can attempt to control for such confounding variables when evaluating associations by using a multilevel model.
...
The response variable we model for is $ln(citations)$ instead of citations, because of the long tail of their distribution.
We also omit the `r sum(by_paper$citations == 0)` papers with zero citations to improve the linear fit with the predictors.

`r epsilon <- 0; clean <- filter(by_paper, citations > 0); model0 <- lm(data = clean, log(citations + epsilon) ~ artifact_released)`
...
of the variance in citations (Adjusted
$R^2$=`r round(summary(model0)$adj.r.squared, 3)`).
...
`r round(coef(summary(modelBIC1))[2,1], 2)`  (`r round(100 * (exp(coef(modelBIC1)[2]) - 1), 0)`%
relative citation increase)
...
Only the maximum h-index and top-university affiliation had statistically significant coefficients, but hardly affected the overall model.^[Note that the past publication counts and h-index are correlated
...
There are numerous factors that authors cannot easily control, such as their own demographic factors, but fortunately, these turn out to have insignificant effects on citations.
...
Other potential implications of our findings mostly agree with our intuition and with previous findings in related studies, as described in the related-work section.
...
There is currently no automated or even manual methodology to reliably discover which papers shared artifacts, how they were shared, and how long did they survive.
...
This possibility certainly opens up an interesting question for future research, using a citation database with reliable self-citation information (unlike GS).
...
Among the many paper-related factors studied in relation to citations is the paper's text availability, which our data show to be also linked with artifact availability.
...
While many obstacles to the universal sharing of artifacts still remain, the field of computer systems does have the advantage that many----if not most---of its artifacts come in the form of software, which is easier to share than artifacts in other experimental fields.
...
That said, a high artifact sharing rate is not enough for the goals of reproducible science, since many of the shared artifacts in our dataset have since expired or have been difficult to locate.

Our analysis found that both the findability and accessibility of systems artifacts can decay significantly even after only a few years, especially when said artifacts are not hosted on dedicated open and free repositories.
