...
In an attempt to standardize expectations for computer-science artifacts, the ACM describes its initative to specifically evaluate artifact availability and functionality in some of its journals and conferences, and award papers with badges accordingly [@ACM20:artifact].
...
It also provides valuabe author feedback [@krishnamurthi15:real].

There are some encouraging signs that such initativies are leading to increased adoption.
...
Nevertheless, a few questions, inpired by similar studies, could be formulated before this data exploration and then be answered by it, we are offered as secondary contributions:
...
A related question to artifact accessibility is how accessible is the actual paper that introduced the artifact, which may itself be associated with higher citations [@mccabe15:availability; @mckiernan16:point; @tahamtan16:factors].
...
## A deeper dive into citation analysis
...
As we have seen, many factors are associated with artifact availability, but they could also be affecting paper citations.
...
One such critical factor is obviously time since publication, which we control for by measuring all citations at exactly the same interval, `r how_long` months since the conference's official start.
...
We can control for the paper-related features we collected by building a sinmple linear regression model of citations as a function of artifact availability, and then add control variables as predictor variables and observe the effect on the main predictor.
In the simple model,
`r model0 <- lm(data = by_paper, citations ~ artifact_released)`

Similarly, some author-related factors such as their own reputation, number of coauthors, author country, and their gender has been associated with citation count [@tahamtan16:factors].
We enhance our linear model with the following metrics:

Conference- or journal-related factors can clearly also have an effect on citations, for example the conference's own prestige and competitiveness, as discussed in the previous section.
Although we can approximate some of these factors, there may also be other unknown or qualitative conference factors we cannot model.
To account for conference effects, we instead build a mixed-methods model, where all the previously mentioned factors become fixed effects and the conference becomes a random effect.
...
Number of citations is a contentions metric,  and it is associated with many other factors, primarily amongst those--we can hope--is the subjective quality of the work.
...
It is therefore encouraging to see an increasing number of systems conferences actively encouraging and investing effort in the artifact evaluation and dissemantion process.
For example, The automated archiving of artifacts by ACM and the inclusion of badges may be inducive to higher artifact availability, but not necessarily to higher citations for these paper.
