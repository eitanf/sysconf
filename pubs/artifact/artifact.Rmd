...
Unfortunately, the sharing and evaluation of artifacts is still not as commonplace as warranted by their importance.
...
This paper aim to shed some light on artifact sharing in one particular field of CS, namely *computer systems* (or "systems" for short).
...
Software artifacts are therefore paramount to the evaluation and reproduction of systems research.
...
An additional goal of is an exploratory data analysis of associated factors to identify relationships between the availability of software artifacts in systems research papers and other statistical properties of these papers.

## Study design and main findings {-}

To evaluate our main hypothesis, this study uses an observational, cross-sectional approach, analyzing `r sum(all_confs$npapers)` papers from a large subset of leading systems conferences.
...
Among other characteristics, it includes manually collected data on artifact availability and paper citations counts 3.5 years from publication, as detailed in the next section.
...
Even after controlling for demographic, paper, and conference factors using a multilevel mixed-effects model, papers with artifacts still receive 34% more citations on average than paper without.

Our expansive dataset also offers the opportunity for a descriptive and correlational study of the following additional research questions, which are provided as secondary contributions:
...
As a final contribution, this study provides a rich dataset of papers, tagged with varied metadata from multiple sources, including for the first time artifact properties (described next).
Since comprehensive data on papers with software artifacts is not always readily available, owing to the significant manual data collection involved, this dataset can serve as the basis of additional studies.
...
The discussion section that follows dives deeper into the confounding factors that also affect paper citations and incorporates them in a mixed-effects multilevel model of citations.
...
tmp <- all_confs %>%
  ...
  mutate(Acceptance = ifelse(is.na(all_confs$acceptance_rate), "Unknown", round(acceptance_rate, 2))) %>%
...
A few of these conferences, such as MobiCom and SLE, specifically encouraged artifacts in their call-for-papers or web sites.
...
When skimming these paper, several search terms were used to assist in identifying artifacts, such as "github,"  "gitlab," "bitbucket," "sourceforge," and "zenodo" for repositories; variants of "available," "open source," and "download" for links; and  variations of "artifact," "reproducibility," and "will release" for indirect references.
...
It is worth noting, however, that most of the source-code repositories in these artifacts showed no development activity---commits, forks, or issues---after the publication of their paper, suggesting limited impact for the artifacts alone.

## Limitations {-}

The methodology described here is constrained by the manual curation of data, especially artifact data.
...
Before addressing our main research question, we start with simple characterization of the statistical distributions of artifacts in our dataset.
...
Finally, we can also break down the citations per paper grouped by the type of location for the artifact and by organization, examining medians because of the outsize effects of outliers (Table \@ref(tab:location-cites)).
...
An interesting tangential question is how often do systems papers cite other artifacts.
...
Incidentally, papers with released artifacts also tend to incorporate significantly more references themselves
(mean: `r report_means("references")`).
However, there is no reason to suspect a causal relationship to artifacts, as opposed to some other confounding cause.
...
In this section we revisit in depth our primary research interest: the effect of artifact sharing on citations.
...
These confounding factors could provide partial or complete statistical explanation to differences in citations beyond artifact availability.
...
This association may still not suffice to claim causation due to hidden variables [@lewis18:open], but it supports the hypothesis that releasing artifacts can indeed improve the prospects of a systems research paper to achieve wider acceptance, recognition, and scientific impact.
...
Artifact sharing invariably accompanies the publication of a paper, while its citations invariable follow months or years afterwards.
...
Such results have lead more CS organizations and conferences to increase adoption of artifact sharing and evaluation, including a few of the conferences evaluated in this paper [@baker16:digital; @dahlgren19:getting; @hermann20:community; @saucez19:evaluating].
...
While many obstacles to universal sharing of artifacts still remain, the field of computer systems does have the advantage that many----if not most---of its artifacts come in the form of software, which is much easier to share than artifacts in other experimental fields.
