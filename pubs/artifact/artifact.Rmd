...
When skimming these papers, several search terms were used to assist in identifying artifacts, such as "github,"  "gitlab," "bitbucket," "sourceforge," and "zenodo" for repositories; variants of "available," "open source," and "download" for links; and  variations of "artifact," "reproducibility," and "will release" for indirect references.
...
2. Also from these sources, manually copy the following information for each paper: title, author names, and award status (as noted on website and in proceedings).
...
6. Collect and record GS citation counts for each paper as close to possible to exactly `r how_long` months after the conference's opening day.

7. Record artifact availability and links for papers. This is likely the most time-consuming and error-prone process in the preparation of the data specific to this study and involves the following steps: Using a search function on each document ("pdfgrep") on each of search terms listed above and reading the results to identify any links or promises to artifacts; skimming or reading papers with negative results to ensure such a link was not accidentally missed; of the surviving negative results, if a paper describes a names system, search github.com for this name or the author names to try to discover unlinked artifacts.
...
Perhaps the greatest challenge in performing this study or in replicating it is the the fact that good metadata on research artifacts is either nonexistent or nonstandard.
...
There are currently several efforts underway to try to standardize artifact metadata and citation, but for this current study, the validity and scalability of the analysis hinges on the quality of the manual process of data collection.
