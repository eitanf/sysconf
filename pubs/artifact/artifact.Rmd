...
Many scientific experimental results cannot be successfuly repeated or reproduced, leading to the so-called "reproducibility crisis" [@baker16:reproducibility; @van15:sluggish].
...
In addition to their benefits for open and reproducible science, papers with linked artifacts haev been sometimes associated with higher citation counts [@childers17:artifact; @heumuller20:publish].
...
Citations may also stand in as a proxy metrics for the transparency, reusability, reproducibility, and credibility of papers, if we make the assumption that any these qualities encourage conbsequent researcher to cite the original work.
...
With this expansive dataset, we can broaden the scope of our exploration in to uncover more relationships between artifact properties and other paper properties.
Despite bing primarily exploratory in nature, this paper still poses and answers  as secondary contributions a few concrete research questions, inspired by similar studies:
...
Citation metrics typically lag pubblication by a few months or years, allowing for the original papers to be discovered, read, cited, and then the citations themselves published and recorded.
...
had not been cited at all at the time of this writing, while several others already garnered hundres of citations.
...
Likewise, the `r nrow(released)` papers that released an artifact garnered more citations than the `r nrow(unreleased)` papers that did promise an artifact that could later not be found
...
This association may still not suffice to claim causation [@lewis18:open], but it supports the hypothesis that releasing artifacts can indeed improve the prospects of a systems research paper to achieve wider acceptance, recogniction, and scientific impact.
