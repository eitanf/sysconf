...
The dataset we analyze comes from a hand-curated collection of `r length(unique(cites$conf))` peer-reviewed systems conferences from a single publication year (2017) to reduce time-related variance.
...
In our dataset of systems papers, only `r sum(gs5y$citations == 0)` papers remain uncited after five years
...
(intercept: `r round(fit$coefficients[1], 2)`, slope: `r round(fit$coefficients[2], 2)` citations per month).
...
(`r fmt(nrow(filter(gs, months==6, citations==0)))`)
...
In our dataset, it averages about `r round(mean(first$Months))` months.
...
veloc %>%
  ggplot(aes(x=months, y=velocity, group=key)) +
...
```{r self-citation-ratio, echo=F, warning=F, message=F, cache=T, out.width='0.75\\textwidth', fig.cap="Mean ratio of self-citations out of total citations as a function of months since publication. Trend lines are smoothed using LOESS. Data are divided into four quartiles based on how many total citations the paper had after 5 years (Q4 means most cited papers)."}
...
Several of those concluded that Google Scholar both has higher coverage and more liberal policies for defining what constitutes a citation [@halevi17:suitability], resulting in higher citation counts overall, as we have also found [@kousha07:google].
...
Of particular relevance, however, we can find similar studies for other fields of computer science [@broch01:cite; @lister08:citation; @wang16:cloud].
Even specific systems subfields and systems conferences were analyzed, such as performance evaluation [@frachtenberg22:sigmetrics], networking [@iqbal19:sigcomm; @iqbal19:bibliometric], cloud computing [@khan22:cloud], and databases [@rahm05:citation].
