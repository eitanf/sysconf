...
subfield_summary <- gen_auths %>%
  group_by(subfield) %>%
  mutate(FAR = sum(gender == "F") / n()) %>%
  ungroup() %>%
  group_by(subfield, FAR, gender) %>%
  summarize(.groups = "keep", aa = mean(as_author)) %>%
  group_by(subfield, FAR) %>%
  summarize(.groups = "keep", prod_adv = nth(aa, 2) / nth(aa, 1))
```
...
In this section we dive deeper into the data by exploring the relationships between the different measurements and metrics.
...
Aggregating the data on a subfield basis (Table \@ref(tab:subfield-aggregation)) confirms that the correlation between a subfield's FAR and its male productivity advantage, defined as the ratio between the mean productivity of men and women, is indeed fairly weak
(`r report_test(cor.test(subfield_summary$FAR, subfield_summary$prod_adv))`).
Some subfields, like theory, exhibit both a high male-to-female productivity ratio and low FAR, and vice-versa, like CS education. 
Others show no apparent relationship, like information retrieval and neural networks that have nearly identical FARs but very dissimilar productivity gaps.

```{r subfield-aggregation, echo = F, cache = T}
subfield_summary %>%
  rename("Productivity ratio" = prod_adv) %>%
  arrange(desc(FAR)) %>%
  knitr::kable(format = "latex",
               booktabs = T,
               align = "lrr",
               linesep = "",
               caption = "Comparison of subfields by different gender metrics, ordered by FAR. Ratios (of mean papers per author) are calculated with women in the denominator.") %>%
  kable_styling()
```
