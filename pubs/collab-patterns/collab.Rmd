...
simplesummary: |
  ...
  help us understand better and possibly address the factors associated with the
...
abstract: |
  ...
  somewhere in the 15%--30% range. Many explanatory factors have been studied
  in association with this gender gap, including differences in collaboration patterns.
  In this study we extend this body of knowledge by looking a differences in collaboration
  ...
  Our main findings are that the gender gap varies greatly by field, averaging about 14%
  and ranging from 6% female authors in theoretical CS to 42% in CS education.
...
acknowledgement: |
  We gratefully acknowledge the assistance of Alexis Richter, Josh Reiss, and Rhody Kaner in collecting
  some of the gender data for this study. Some of these students have been supported by grants
  from the Reed Social Justice Research and Education Fund.
authorcontributions: |
...
Related, another study of collaboration patterns across sciences found that CS papers average 2.84 coauthors, and electrical engineering papers average 3.04 [@ghiasi15:compliance], somewhat lower than what we have found in this study.
...
In this study, we do not aggregate papers by gender, unless they have a single author, in which case there is only one gender.
...
To verify the accuracy of our approach, we compared our manually assigned genders to self-assigned binary genders in a separate survey we conducted among 918 of the authors  [@frachtenberg20:survey].
...
Of the fields that are more affected by this metric, the most notable perhaps is theoretical computer science, which now exhibits the highest mean fractional productivity.
...
This correlation is even weaker when using fractional paper counts, since the slightly larger team sizes for men attenuate their productivity advantage
...
We can extend this analysis of correlation with FAR to three other research questions we asked.
...
(`r report_test(cor.test(subfield_summary$FAR, subfield_summary$single_ratio))`), so we must reject this hypothesis.
...
We therefore look at "women's homophily", defined as the ratio between a subfield's probability for a woman in to coauthor with a woman and its FAR.
...
This high skew towards men also means we cannot use standardized metrics for homophily such as Krackhardt and Stern's E-I Index [@krackhardt88:informal], defined simply as
...
One such example is theoretical computer science, which shows values on one extreme or another in many of the metrics we collected.
...
Theoretical CS is an extreme point by almost all metrics, and matches what  found for math.
...
As discussed in RQ6, one possible explanation for the large gender gap is the high cost in participation in experimental fields such as systems, which has been associated with lower inclusivity [@duch12:possible].
...
We can certainly observe corroboration for this hypothesis in our data, with CS education and HCI as the top two fields in FAR, followed by WWW and multimedia, which also put the human in the center of the research.
...
Many papers exhibit larger team sizes than is typical in most other scientific disciplines, and very few papers are written by a single author.
Understanding collaboration patterns in CS authorship is therefore critical to understanding related bibliometric and societal phenomena, such as the large gender gap in CS.
...
Fields that exhibit lower representation of women than average also tend to exhibit larger team sizes, smaller cohort sizes for women, higher gender homophily, and higher author productivity.
...
All of these associations have exceptions, and perhaps the most notable is the subfield of theoretical CS, that stands alone in the extreme of most metrics, perhaps being better categorized as a subfield of mathematics than of CS.
On the opposite end, CS subfields that have increased focus on people rather than computers, such as CS education and HCI, show better overall representation of women and less extreme gaps cross most metrics.
...
This last field of computer systems is of particular interest to us, because of its large size, its impact on technology and the economy, and its very low representation of women, at about 10%.
We will therefore focus our attention next not just on statistical association with low FAR, but on causal association.
...
It could be instructive to follow up on this study with data collected from later years to observe any trends and changes in collaboration patterns and representation of women.
