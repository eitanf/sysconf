...
collaborator_proportions %>% 
  ...
  group_by(gender) %>% 
  summarise(
    avg_n_collaborators = round(mean(total_collaborators, na.rm = T), 2)
    ) %>% 
  knitr::kable()
...
collaborator_proportions %>% 
  ...
  group_by(gender) %>% 
  summarise(
    avg_n_female_collaborators = round(mean(n_female, na.rm = T), 2),
    avg_n_male_collaborators = round(mean(n_male, na.rm = T), 2)
    ) %>% 
  knitr::kable()
```
