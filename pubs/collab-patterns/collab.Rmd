...
subfield_summary <- gen_auths %>%
  group_by(subfield) %>%
  mutate(FAR = sum(gender == "F") / n(), avg_prod = mean(as_author), avg_coauthors = mean(coauthors)) %>%
  ungroup() %>%
  group_by(subfield, gender, FAR, avg_coauthors, avg_prod) %>%
  summarize(.groups = "keep", aa = mean(as_author), co = mean(coauthors)) %>%
  group_by(subfield, FAR, avg_coauthors, avg_prod) %>%
  summarize(.groups = "keep",
            prod_ratio = nth(aa, 2) / nth(aa, 1)) %>%
  left_join(tmp) %>%
 ...
 *  **RQ4**: Do authors exhibit gender homophily in their choice of coauthors?
...
For this specific study, we decided to expand the analysis to include some of the most influential conferences in most subfields of CS, based on the same measures, for a total of `r n_distinct(full_auths$key)` papers acorss CS.
...
We follow the approach of Wang et al. to estimate whether authors collaborate with coauthors of the same gender at rates higher than expected [@wang21:trends].
...
Next, we turn our attention to single-author papers.
...
```{r solo-props-by-subfield, echo=F, warning=F, message=F, cache=T, fig.height=6.5, out.width='0.75\\textwidth', fig.cap = "Percentage of papers in each subfield that were written by a single author."}
full_auths %>%
  group_by(subfield) %>%
  mutate(N = n(), avg_solo = sum(coauthors == 0) / n_distinct(key)) %>%
  ungroup() %>%
...
Fig. \@ref(fig:solo-props-by-subfields) shows for each subfield what percentage of all papers was written by single authors, which is lower than 5% for most subfields.
For this metric, breakding down the data by gender makes little sense, because a multi-author paper has no "gender" and because the numbers of single-author papers per subfield are too small for statistical significance.
In fact, the numbers of single-authors papers are so low, as is the number of women authors overall, that their intersection is actually empty for most subfields.
That said, women publish fewer single-author papers than men in all but three of the subfields: WWW, Data Science, and Storage.
In the latter, there was only one paper written by a single author, which happens to be a woman.

It is also worth noting that in many of the systems subfield the percentage of single-author papers is extremely low, and is zero in five of those.
This obersvation may be another indication that the systems field in particular depends on larger collaboration teams for published research.
...
The data we collected on coauthorship (Fig. \@ref(fig:coauthors-by-subfield))
...
The first relationship we investigate is between productivity (RQ2) and FAR.
...
Obviously, the observation that men publish more than women implies that we would find more names of men on papers than we would of women, but the modest advantage in overall productivity for men
(`r round(mean(100 * filter(gen_auths, gender == "M")$as_author) / mean(filter(gen_auths, gender == "F")$as_author) - 100, 2)`%) cannot fully explain the nearly 9:1 ratio of male-to-female authorship.
...
`r tmp <- gen_auths %>% group_by(gender) %>% summarize(.groups = "keep", names = n_distinct(name)); pct(filter(tmp, gender=="F")$names, sum(tmp$names))`% women overall.
While this ratio represents a significant improvement over the non-unique FAR, it is still far from parity, suggesting that higher productivity alone cannot fully explain the gender gap.
Aggregating the data on a subfield basis confirms that the correlation between a subfield's FAR and the mean productivity of its practitioners is fairly weak
(`r report_test(cor.test(subfield_summary$FAR, subfield_summary$avg_prod))`).

The correlation between FAR and male productivity advantage, defined as the ratio between the mean productivity of men and women, is even weaker overall
(`r report_test(cor.test(subfield_summary$FAR, subfield_summary$prod_ratio))`).
Some subfields show no apparent relationship between the metrics, like information retrieval and computer vision, which have nearly identical FARs but very dissimilar productivity gaps.
Other subfields exhibit a stronger opposite relationship, like theory one extreme of the FAR data, with both a high male-to-female productivity ratio and low FAR, and vice-versa, like CS education on the other extreme.

```{r subfield-aggregation, echo = F, cache = T}
rounding = 3
data.frame(Subfield = subfield_summary$subfield,
           FAR = round(subfield_summary$FAR, rounding),
           "Mean productivity" = round(subfield_summary$avg_prod, rounding),
           "Productivity ratio" = round(subfield_summary$prod_ratio, rounding),
...
These two subfields on the extreme also serve as a good counter-example to a hypothetical relationship between FAR and the typical collaboration size in the subfield (RQ3).
Both exhibit a relatively low metric for mean coauthors per paper, but with very different FAR metrics.
Here too, the negative correlation between the two metrics overall is quite low and even less significant.
(`r report_test(cor.test(subfield_summary$FAR, subfield_summary$avg_coauthors))`).

Next on our list of comparisons is gender homophily (RQ4).
