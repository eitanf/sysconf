...
{
  men <- filter(full_auths, gender == "M", subfield == sf)
  women <- filter(full_auths, gender == "F", subfield == sf)
...
We may need "RQ0": distribution of FAR across conferences, subfields, fields.
...
Here, we start our exploration of the data by looking at the productivity rates across genders and subfields of CS.
...
`r pct(nrow(filter(full_auths, gender == "F", as_author == 1)), nrow(filter(full_auths, gender == "F")))`%
of female authors published only one paper in our dataset, compared to
`r pct(nrow(filter(full_auths, gender == "M", as_author == 1)), nrow(filter(full_auths, gender == "M")))`%
of men.

Overall, men average
`r round(mean(filter(full_auths, gender == "M")$as_author), 2)`
papers per author, compared to women's
`r round(mean(filter(full_auths, gender == "F")$as_author), 2)`
(`r report_test(t.test(filter(full_auths, gender == "F")$as_author, filter(full_auths, gender == "M")$as_author))`).
...
(`r report_test(wilcox.test(filter(full_auths, gender == "F")$as_author, filter(full_auths, gender == "M")$as_author))`).
...
```{r echo=F, eval=F}
for (sf in levels(full_auths$subfield)) { m<-filter(full_auths, gender == "M", subfield == sf); f<-filter(full_auths, gender == "F", subfield == sf); print(paste(sf, mean(m$as_author)-mean(f$as_author), report_test(t.test(m$as_author, f$as_author)))) }
```
