...
full_auths %>%
  ...
  ggplot(aes(x = as_author, fill = gender)) +
    ...
    scale_fill_manual(values = c(cwomen, cmen), labels = "Women", "Men") +
...
```{r productivity-by-subfield, echo=F, warning=F, message=F, cache=T, out.width='0.95\\textwidth', fig.cap="Distribution of number of distinct papers per author"}
full_auths %>%
  drop_na(gender) %>%
  group_by(subfield) %>%
