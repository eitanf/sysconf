...
coauthors <- readr::read_csv("../../features/coauthors.csv")
persons <- readr::read_csv("../../features/persons.csv")
roles <- readr::read_csv("../../features/roles.csv")
confs <- readr::read_csv("../../features/confs.csv")
...
sys_auths <- roles %>% 
  ...
  dplyr::select(name, gender, key) %>% 
  mutate(conf = str_extract(key, ".+?(?=_0)")) %>% 
  left_join(confs, by = c("conf" = "conference")) %>% 
...
full_auths <- rbind(sys_auths, nonsys_auths)
