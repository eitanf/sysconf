...
nonsys_full_table <- nonsys_authors %>%
  ...
  dplyr::select(conf, subfield, n_papers, acceptance_r) %>%
...
sys_full_table <- all_confs %>%
  ...
  dplyr::select(Conference, subfield, Papers, "Acceptance rate")

full_table <- as.data.frame(rbind(nonsys_full_table, sys_full_table)) %>%
  arrange(subfield, `Acceptance rate`)
...
Many papers across disciplines discuss the existence and potential reasons for a productivity gap, where men generally publish more scholarly articles than women.
