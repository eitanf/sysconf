...
Although this productivity metric appears to accentuate gender gaps in productivity, it does not appear to affect the relative rankings of subfields by productivity very much.
...
As the data in Table \@ref(tab:single-authors) shows, in most CS subfields, fewer than 5% of papers were written by a single author, averaging only `r df[nrow(df),4]`.
...
df <- bind_rows(
  ...
  data.frame(check.names = F,
    ...
    "Fractional productivity" = c(round(mean(full_auths$avg_frac), rounding),
...
df %>%
  knitr::kable(format = "latex",
               ...
               caption = "Comparison of subfields by different gender metrics, ordered by FAR. Metrics include mean productivity (papers per author), fractional paper total, mean total coauthors (with repeats), the ratio between a woman's probability to coauthor with a woman and FAR.") %>%
