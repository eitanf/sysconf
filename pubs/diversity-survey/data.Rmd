...
(Pearson's `r report_stat(cor.test(authors_per_paper$app, conf_counts$Response / tmp$Papers), 2)`).
...
(`r dist_survey <- survey %>% group_by(paper_id) %>% summarize(nauthors = n()); dist_authors <- roles %>% filter(role == "author") %>% group_by(key) %>% summarize(nauthors = n()); report_stat(t.test(dist_authors$nauthors, dist_survey$nauthors), 2)`), 
