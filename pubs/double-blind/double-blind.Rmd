...
all_good <- ppl %>%
  filter(!is.na(npubs), !is.na(s2npubs), npubs / s2npubs < THRESH_RATIO, s2npubs / npubs < THRESH_RATIO) %>%
  group_by(name, gs_email) %>%
  summarize(npubs = first(npubs), s2npubs = first(s2npubs), hindex = first(hindex))
...
ppl.best_source <- ppl %>%
  mutate(npubs = ifelse(good_npubs, npubs, predict(model.npubs_from_s2npubs, data.frame(s2npubs = c(s2npubs))))) %>%
  mutate(hindex = ifelse(good_npubs, hindex, predict(model.hindex_from_s2npubs, data.frame(s2npubs = c(s2npubs)))))
...
With NA's dropped, npubs:

 * Maximum prior publications (most prolific author): `r report_db(ppl.filtered, "max_npubs")`
 * Minimum prior publications (least prolific author): `r report_db(ppl.filtered, "min_npubs")`
 * Mean prior publications: `r report_db(ppl.filtered, "mean_npubs")`
 * Median prior publications: `r report_db(ppl.filtered, "median_npubs")`
 * Total prior publications: `r report_db(ppl.filtered, "total_npubs")`
 
 -----
 
With NA's dropped, Hindex

 * Maximum H-index (most senior author): `r report_db(ppl.filtered, "max_h")`
 * Minimum H-index (least senior author): `r report_db(ppl.filtered, "min_h")`
 * Mean H-index: `r report_db(ppl.filtered, "mean_h")`
 * Median H-index: `r report_db(ppl.filtered, "median_h")`
 * Total H-index: `r report_db(ppl.filtered, "total_h")`

 -----
 
With NA's flattened down, Hindex:

 * Maximum H-index (most senior author): `r report_db(ppl.flat, "max_h")`
 * Minimum H-index (least senior author): `r report_db(ppl.flat, "min_h")`
 * Mean H-index: `r report_db(ppl.flat, "mean_h")`
 * Median H-index: `r report_db(ppl.flat, "median_h")`
 * Total H-index: `r report_db(ppl.flat, "total_h")`

 -----

With NA's linearly imputed from papers, Hindex:

 * Maximum H-index (most senior author): `r report_db(ppl.linear, "max_h")`
 * Minimum H-index (least senior author): `r report_db(ppl.linear, "min_h")`
 * Mean H-index: `r report_db(ppl.linear, "mean_h")`
 * Median H-index: `r report_db(ppl.linear, "median_h")`
 * Total H-index: `r report_db(ppl.linear, "total_h")`

 -----

With all "bad npubs" linearly imputed from S2 npubs, npubs:

 * Maximum Npubs (most senior author): `r report_db(ppl.best_source, "max_npubs")`
 * Minimum Npubs (least senior author): `r report_db(ppl.best_source, "min_npubs")`
 * Mean Npubs: `r report_db(ppl.best_source, "mean_npubs")`
 * Median Npubs: `r report_db(ppl.best_source, "median_npubs")`
 * Total Npubs: `r report_db(ppl.best_source, "total_npubs")`

 -----

With all "bad npubs" linearly imputed from S2 npubs, Hindex:

 * Maximum H-index (most senior author): `r report_db(ppl.best_source, "max_h")`
 * Minimum H-index (least senior author): `r report_db(ppl.best_source, "min_h")`
 * Mean H-index: `r report_db(ppl.best_source, "mean_h")`
 * Median H-index: `r report_db(ppl.best_source, "median_h")`
 * Total H-index: `r report_db(ppl.best_source, "total_h")`


 -----


Looking at papers in dataset metric only:

 * Maximum papers in dataset: `r report_db(ppl, "max_papers")`
 * Minimum papers in dataset: `r report_db(ppl, "min_papers")`
 * Mean papers in dataset: `r report_db(ppl, "mean_papers")`
 * Median papers in dataset: `r report_db(ppl, "median_papers")`
 * Total papers in dataset: `r report_db(ppl, "total_papers")`
...
Questions:
 ...
 - Who are the people with large discrepancy between pubs and npubs? People with common names that get merged with other researchers for too many pubs; or in GS case, counting many non-paper pubs.
 ...
 - H-index is higher for DB, but npubs is lower? (Correct for conf quality)
 - What is the effect of outlier publishers?
 - There appears to be a prestige bias so that more senior authors (max npubs) are more prevalent in SB conference, and this doesn't seem to change based on how we treat the "bad npubs". But this isn't true for H-index--why?
 - How do we control for conference prestige in this astensible bias?
