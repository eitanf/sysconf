...
```{r experience.setup, echo=F, warning=F, message=F}
experience <- roles %>%
  left_join(persons) %>%
  filter(role == "author") %>%
  drop_na(hindex) %>%
  ...
  summarize(conf = as.character(first(conf)),
            ...
            min_h = min(hindex),
            mean_h = mean(hindex),
            median_h = median(hindex),
            total_h = sum(hindex),
            max_npubs = max(npubs),
            min_npubs = min(npubs),
            mean_npubs = mean(npubs),
            median_npubs = median(npubs),
            total_npubs = sum(npubs)) %>%
  left_join(select(all_confs, c(key, double_blind)), by = c("conf" = "key"))

report_db <- function(metric) {
  sb <- filter(experience, double_blind == F)
  db <- filter(experience, double_blind == T)
  paste("Mean SB:", round(mean(sb[[metric]]), 2),
 ...
 * Maximum prior publications (most prolific author): `r report_db("max_npubs")`
 * Minimum prior publications (least prolific author): `r report_db("min_npubs")`
 * Mean prior publications: `r report_db("mean_npubs")`
 * Median prior publications: `r report_db("median_npubs")`
 * Total prior publications: `r report_db("total_npubs")`
 
 -----
 
 * Maximum H-index (most senior author): `r report_db("max_h")`
 * Minimum H-index (least senior author): `r report_db("min_h")`
 * Mean H-index: `r report_db("mean_h")`
 * Median H-index: `r report_db("median_h")`
 * Total H-index: `r report_db("total_h")`
 ...
 * [nicholas17:early] Early career researchers appear to have a positive experience overall with peer review.
 ...
 * [@ho13:views] surveyed researchers and found that native English speakers and women found peer review more fair than others.
