...
# other source
cleaning_imputation <- function(dat, THRESH_RATIO, S2_THRESH){
  
  dat <- dat %>% 
    mutate(imp = is.na(npubs) | npubs/s2npubs >= THRESH_RATIO & s2npubs <= S2_THRESH) %>%
    mutate(npubs_all_good = na_if(npubs / s2npubs < THRESH_RATIO |
                                    s2npubs / npubs < THRESH_RATIO, npubs))

    
  # Create training dataset for imputation
  dat_train <- dat %>%
  filter(!imp, !is.na(s2npubs), s2npubs/npubs < 3, s2npubs < S2_THRESH) %>%
  ...
  mutate(npubs_imp = ifelse(!imp, npubs, predict(model.npubs_from_s2npubs,
                                                 data.frame(s2npubs = c(pmin(s2npubs, S2_THRESH)))))) %>%
  ...
  who %>%
    ...
    summarize(conf = as.character(first(conf)), 
              ...
              across(c(as_author, as_pc, hindex, i10index, citedby, paper_cites, npubs, s2npubs,
                       npubs_imp, hindex_imp, imp, npubs_flat,
                       hindex_flat),
    ...
    mutate_all(.funs = function(x) ifelse(is.infinite(x) | is.nan(x), NA, x)) %>%
                          ...
                          )
{
  ...
  experience <- who %>%
    group_by(as.factor(key)) %>%
    summarize(conf = as.character(first(conf)),
              prestige = ifelse(logar, fun(log(!!rlang::sym(metric) + 1)),
                                fun(!!rlang::sym(metric))),
              .groups = 'drop')  %>%
    left_join(all_confs, by = c("conf" = "key"))
...
########################
# compute_db: for a given population and metric, return a vector with different statistics on
# the difference of means and medians for the metric between SB and DB
...
# the regression of the metric by double-blindess (linear and logistic).
...
compute_glm <- function(exper = experience.all, predictor = "", metric = "npubs")
{
  ...
  form = as.formula(paste(col, "~", predictor, ifelse(predictor == "", "", "+"), "!double_blind"))
...
# For a given metric met in a data frame df, compute the maximum absolute value of given column
...
We therefore also considered another proxy metric for author experience (total number of past publications) from another source, the Semantic Scholar database [@XXX:cite].
...
For statistical testing, group means were compared pairwise using Welch's two-sample t-test; differences between distributions of two categorical variables were tested with the $\chi^{2}$ test; and comparisons between two numerical variables were evaluated with Pearson's product moment correlation coefficient. All statistical tests are reported with their p-values.
...
To prepare our data for analysis, we must explore the quality of each variable and observation and then to *clean up* or transform the data. This key, and time-consuming, step does not follow a formal set of rules but instead usually requires the researcher to make several subjective choices, each of which potentially impacts the final results.  Since the primary goal of this paper is to explore the sensitivity of the relationship between review policy and prestige bias to analysis choices, we explore key data cleaning operations and their impact in this section.

To do so, we interrogate the author-level dataset where each row represents one of the `r distinct(select(ppl, name, gs_email)) %>% nrow()` distinct authors.  Figure \@ref(fig:metric-distribs) displays the distributions of the various author-level metrics that serve as our potential proxies for author prestige.  While Section \@ref(sec:results) thoroughly explores the merits and weaknesses of each of these metrics, here we examine the quality of the data representing these variables.  All metrics are fairly right skewed and there are reasons to suspect the quality of the values in both the upper and lower tails of these distributions. Because of name disambiguation issues, the upper tails are likely inflations of their true values for many authors.  We decided not to winsorize these large values or to remove them from the dataset since we could not find a systematic issue.  Insted, we expanded our analyses in Section \@ref(sec:results) to include a Wilcoxon Rank Sum Test, which is resistant to outliers.
...
In terms of the lower tail, it should be noted that about one third of the values for the GS measures are missing since those authors did not have a GS profile or the profile appeared to aggregate more than one person.  Since the median number of publications from SS is only `r filter(ppl, is.na(npubs)) %>% summarize(median(s2npubs)) %>% pull()` for those without a GS measures but equals `r filter(ppl, !is.na(npubs)) %>% summarize(median(s2npubs)) %>% pull()` for those with a GS measures, these authors with missing values are likely less experiences and therefore would contribute to the lower tail of these distributions.  The variation in SS publications between those with and without GS profiles provides evidence that the missing values are not missing at random and that we should determine how sensitive our results are to how the missingness is treated.  

For handling these missing values, we consider three strategies.  The simplest approach involves removing the missing values when computing aggregate measures. This strategy likely inflates paper aggregation metrics such as the mean but has less influence on the maximum metric.  On the other end of the spectrum, we also consider imputing the missing metrics with the minimum value for that metric.  This assumes that the missing values all represent new researchers and will pull down the mean paper aggregation metrics but again have less influence on the maximum metric.  Lastly, we consider imputing the missing values based on a linear regression model built on the total publications from SS, which is moderately correlated with the GS metrics (Fig. \@ref(fig:metric-corrs)). For the linear imputation, we also imputed the GS metrics if the ratio of the total publications from GS compared to SS was greater than 3, which indicated a potential inflation of the values of GS metrics for those authors (Fig. \@ref(fig:imputation)).  Similarly, to minimize imputing inflated GS metrics, we winsorized at 1000 the very heavy tail of the total publications from SS before computing the imputed values.  XXX: Insert some summary statements about the impact of these three strategies.  For nearly all of the results, the sign of the relationship between review policy and the prestige metric does not change but in some cases the degree of significant does change.  Generally, it appears that imputing the missing data doesn't have a big impact on prestige bias analysis because most of the missing data are for the less experienced authors, who don't impact many of our aggregation metrics.  Therefore, for the rest of the paper, we will use the unimputed data and remove missing values when computing aggregate measures.




```{r metric-corrs, echo=F, warning=F, cache=T, fig.cap="A graphical display of the lower triangle of a correlation matrix for the author-level prestige metrics.", fig.height = 8}
cors <- ppl %>%
...
corrplot(cor(cors, use = "pairw"), type = "lower", order = "hclust", 
...
```{r imputation, echo = FALSE, message = FALSE, warning= FALSE, fig.cap = "Scatterplot of the relationship between the two author-level, total publication metrics.  The estimated linear regression line used for imputation is included in black and pointed where the GS metric is 3 times the SS metric are highlighted."}

ppl %>%
  select(npubs, s2npubs, imp) %>%
  filter(s2npubs < S2_THRESH) %>%
ggplot(aes(y = npubs, x = s2npubs)) +
  geom_point(alpha = 0.1, mapping = aes(color = imp)) +
  geom_smooth(method = lm, se = FALSE, color = "black") +
  guides(colour = guide_legend(override.aes = list(alpha = 1))) +
  labs(x = "Publications (SS)", y = "Publications (GS)", color = "Imputed?")
...
In this section we explore the observed relationships between reputation and reviewing policy across three design dimensions, or factors: metrics of reputation, aggregation of prestige across coauthors, and additional conference variables that could affect this relationship. Because of the complexity introduced by multiple independent factors, we start with a common set of choices for comparisons and investigate the effect of each factor and level in relation to this baseline. The baseline we chose is to approximate the reputation of a paper's coauthors by the maximum total number of prior publications of any coauthor, and ignoring any confounding variables. This metric has been commonly used in other studies as well [@fisher94:effects; @madden06:impact; @tomkins17:reviewer; @tung06:impact].
 ...
 * Systems papers in particular vary significantly in numbers of coauthors because some implementations take a large team effort. Credit attribution therefore becomes particularly tricky in this field, even with h-index [@koltun21:hindex].
...
A researcher's participation in these PCs is related to their reputation in two ways.
...
Then, we look at qualitative (discrete) classifications of a paper's potential for recognition.
...
For example, the total reputation of the coauthor group could also play a potential role in the reviewer's bias (ignoring all papers that had one or more missing reputation values, where a partial sum is meaningless).
We can also look at the mean or median reputation (ignoring only missing authors, not entire papers).
Additionally, we can aggregate papers by looking only at the reputation of the first or last author.
Typically, systems papers list the primary contributor in the first author position, while the head of the lab is often listed last; in our dataset, only
`r papers <- left_join(papers, co); p3 <- filter(papers, nauthors > 2); pct(nrow(filter(p3, alphabetized)), nrow(p3), 2)`%
of papers with 3 or more coauthors ordered the author list alphabetically.


```{r mean-diffs, echo = F, warning = F, fig.cap="Test statistics for difference in means with various reputation metrics and aggregations across papers using two-sided Student's t-test. Positive t-statistic values (purple) indicate higher reputation in SB conferences. Darker colors reflect larger-magnitude values, normalized by columns. Negative values (orange) indicate higher reputation in DB conferences. Stars indicate p values below 0.05, 0.01, and 0.001, respectively."}
...
Any statistically sound evidence to suggest bias can find a counter-argument just as sound in this dataset.
 ...
 * However, using the more complete S2 database, which includes many more authors of low experience and reputation, lowers the t-statistic values and significance. This result may be unsurprising, as it incorporates more data, particularly in the lower side of the reputation distribution; but it serves as a cautionary tale for ascribing statistical significance from partial data.
 ...
 * Looking at each paper's median reputation metric instead of mean leads to higher t-statistic values, i.e., higher indicated reputation in SB conferences and higer likelihood of prestige bias. (((WHY?))) The p-values are also lower, likely because the standard error of the median is smaller than that of the mean.
...
for (metric in c("npubs_imp", "as_author", "citedby", "paper_cites", "hindex_imp", "i10index")) {
  for (aggr in c("max", "total", "mean", "median")) {
    tbl <- rbind(tbl, compute_db(experience.all, aggr, metric))
  }
}
...
# Append lead-author stats
for (metric in c("s2npubs", "npubs_imp", "as_author", "citedby", "paper_cites", "hindex_imp", "i10index")) {
  tbl <- rbind(tbl, compute_db(experience.lead, "first", metric))
}
tbl <- ifelse(tbl == "first", "lead", tbl)
...
rep.all <- as_tibble(tbl)
rep.all$metric <- factor(rep.all$metric, ordered = T,
                         levels = c("npubs_imp", "s2npubs", "as_author", "citedby", "paper_cites", "hindex_imp", "i10index"),
                         labels = c("Publications (GS)", "Publications (S2)", "Papers in set", "Total citations", "Citations per paper", "h-index", "i10-index"))
rep.all$aggregation <- factor(rep.all$aggregation, ordered = T,
                              levels = c("lead", "last", "median", "mean", "total", "max"),
                              labels = c("Lead author", "Last author", "Median", "Mean", "Total", "Maximum"))
...
for (metric in c("npubs_flat", "as_author", "citedby", "paper_cites", "hindex_flat", "i10index")) {
  for (aggr in c("max", "total", "mean", "median")) {
    tbl <- rbind(tbl, compute_db(experience.all, aggr, metric))
  }
}
...
# Append lead-author stats
for (metric in c("s2npubs", "npubs_flat", "as_author", "citedby", "paper_cites", "hindex_flat", "i10index")) {
  tbl <- rbind(tbl, compute_db(experience.lead, "first", metric))
}
tbl <- ifelse(tbl == "first", "lead", tbl)
...
rep.all <- as_tibble(tbl)
rep.all$metric <- factor(rep.all$metric, ordered = T,
                         levels = c("npubs_flat", "s2npubs", "as_author", "citedby", "paper_cites", "hindex_flat", "i10index"),
                         labels = c("Publications (GS)", "Publications (S2)", "Papers in set", "Total citations", "Citations per paper", "h-index", "i10-index"))
rep.all$aggregation <- factor(rep.all$aggregation, ordered = T,
                              levels = c("lead", "last", "median", "mean", "total", "max"),
                              labels = c("Lead author", "Last author", "Median", "Mean", "Total", "Maximum"))
...
For example, a 2006 study found no significant difference in the mean publications of authors in two conferences [@madden06:impact], but a followup study on the same dataset found evidence for prestige bias when comparing across medians [@tung06:impact].
...
```{r rep-threshold-npubs, echo = F, warning = F, fig.cap = "Proportion of papers that have at least one author with at least as many publications as the threshold (Google Scholar data)."}
...
Comparing these proportions across SB and DB conferences, with $\chi{}^2$-test-derived p-values below 0.05 in `r pct(sum(tbl$p.value < 0.05, na.rm = T), nrow(tbl) - 1, 1)`% of the threshold values.
...
This time, p-values below 0.05 show up for `r pct(sum(tbl$p.value < 0.05, na.rm = T), nrow(tbl) - 1, 1)`% of the threshold values.
...
Another related qualitative approach to look at seniority and experience was described in the original h-index study [@hirsch05:index]. It proposed that researchers may fall into experience bands, roughly corresponding to novice/assistant professor, mid-career/associate professor, and experienced/full professor. Based on his observations of the field of physics, Hirsch suggested h-index limits of up to 11, between 12 and 17, and 18 and over to distinguish among these bands.
...
e_with_seniority %>%
  ...
  ggplot(aes(fill = Experience, x = double_blind)) +
    ...
    theme_minimal() +
    theme(legend.position = "bottom") +
    scale_fill_viridis_d(guide = "Kermit", breaks = c("Novice", "Mid-career", "Experienced")) +
...
Their study tagged a paper as coming from a "top university" if any of its coauthors is affiliated with one of the top 50 univsersities per [www.topuniversities.com](www.topuniversities.com) (27% of papers).
...
(It may be reasonable to include more companies in the definition of "top company" for contemporaneous systems conferences, such as Amazon, Oracle, and Apple, but we kept the same definition for direct comparisons.)
...
Our analysis so far assumes that the reviewing policy of a conference as an independent, random variable.
In this section, we ask the question, how would our findings on prestige bias change if we examined them in a larger context of conference factors?
...
(`r report_test(t.test(db_confs$mean_authors_per_paper, sb_confs$mean_authors_per_paper))`),
...
In other words, DB conferences themselves show several distinct characteristics, some of which suggest better reputation.
...
```{r linear-regression, echo = F, warning = F, fig.cap="Linear regression of various author prestige metrics as predicted by review policy and other conference variables. Values shown are the coefficient of DB reviewing in the linear model and their associated p-values. Positive coefficients (purple) indicate higher reputation in SB conferences. Negative coefficients (orange) indicate higher reputation in DB conferences. Darker colors reflect larger-magnitude values, normalized by columns. Stars indicate p values below 0.05, 0.01, and 0.001, respectively."}
...
colnames(reg.all) <- c("metric", "predictor", "linear_c", "linear_p", "logistic_c", "logistic_p")
...
To evaluate the effect of these confounding variables, we built simple linear regression models with author prestige as the outcome variable (maximum in paper) and DB reviewing status as the predictor variable (Fig. \@ref(fig:linear-regression)).
The first row corresponds with the first row in Fig. \@ref(fig:mean-diffs) and shows the coefficient of the DB factor alone on author prestige, which often exhibits a statistically strong association.
Each row afterwards includes one of the variables listed above that could stand in for conference prestige as a predictor variable.

Looking first at citation-based author metrics (total citations, citations per paper, and h-index), we can indeed confirm that adding a conference prestige confounder generally lowers the values of the review-policy coefficient and its associated p-value, sometimes even changing signs.
Some of these citation author-conference relationships are almost tautological in nature.
For example, it is not surprising to see people with many total past citations publishing more in confeferences with a higher total past citations, because it is likely that the same highly-cited papers that contributed to a person's count also contributed to the conference's count, assuming some consistency in publishing venues over time.
Such strong associations obviate other explanatory variables such as review status, which is why the coefficient and p-value of DB reviewing drops to near-zero when we correct for conference prestige.

But once more, the picture becomes less clear when we look at the first three columns, representing publication-based author prestige metrics.
...
If anything, the association between past author publications and SB reviewing appears a little stronger when correcting for conference prestige.
...
There is no reason to assume that a person with many past or present publications also has a history of publishing in more competitive conferences.
If anything, we would expect prolific authors to publish less in conferences with a low acceptance rate, simply by making a probabilistic argument.
In that case, we would expect to see even higher coefficients for DB reviewing when correcting for low-prestige conferences, as some of the rows indeed appear to show.
...
In summary, even the effect of correction for confounding variables appears to be senstive to the metrics chosen.
...
(((Does it make sense to use Bayesian regression models instead?)))
To complete our experimental evaluation, it may make sense to treat an author's prestige as a categorical, rather than a continuous variable.
If we treat fame as a boolean factor instead, i.e., an author's name is either recognized or not, then we can repeat our analysis with logistic regression.
We can do so by picking a "fame threshold", above which we consider an author's name to be recognized.
As disscussed in Sec. \@ref(subsec:binary), senior authors show the same preference for DB or SB conferences regardless of the threshold selected to define seniority, at least for h-index and number of publications.
...
Figure \@ref(fig:logistic-regression) depicts the effects of this treatment of author prestige on the association with DB reviewing, corrected for conference prestige.
Directionally, the results are similar to those of the linear regression.
But restricting name recognition to a specific quantile offers two desirable properties for the coefficient comparison: it normalizes all the coefficients to the same range from 0 to 1, and eliminates the outsizes effect of high-prestige author outliers on the linear regression.
Consequently, observing general trends becomes easier and less noisy.

The most noticeable general trend with this classfication of prestige is that most coefficients are now statistically nonsignificant, meaning that it is now harder to claim prestige bias one way or the other, regardless of confounding variables.
...
-- There are two "types" of senior researchers. Type A publish fewer but more cited papers, and type B just publishes a lot of papers, but have lower h-index than type A relative to npubs. The DB conferences show more type-A people, and the SB more type B. The DB conferences also average higher on various prestige metrics and are more competitive. This would be consistent with fewer, more cited papers. But it means you have to correct for conference prestige.
   ...
   * Repeat the statistical argument that prolific authors are less likely to publish in more competitive conferences. Check that by correlating "papers in set" with "acceptance rate" for all authors.
...
-- Our analysis compared dozens of variable combinations, which poses the risks of data mining and "p-hacking".
We therefore suggest not to ascribe too much meaning to individual p-values, but rather look at the big picture: how different perspectives on the same data con support diametrically opposed conclusions.
Our suggestion for similar analyses on prestige bias is to try to analyze and present more than one or two such perspectivies and note any disagreements.
...
-- Recommendation: take "quantity vs. quality" property into account when evaluating author prestige. Do it either by startifying authors into classes, or including mean_citations_per_paper as a confounding variable in any regression, both for max h-index and max npubs. Is that even kosher to do, given that they're collinear?
...
<!--
 ...
 - Compute the % of papers for which max_h and max_npubs are the same person:
 r pct(nrow(filter(experience, max_h_idx != max_npubs_idx)), nrow(experience), 3)%.
 ...
 There are `r ppl %>% group_by(as.factor(key)) %>% summarize(maxhi = which.max(hindex), maxpi = which.max(npubs)) %>% filter(maxhi != maxpi) %>% nrow()` papers where the max npubs author is not the same as the max h-index.
...
Our analysis showed that ...
 ...
 - Add recommendations? perhaps to discussion?
    * For observational studies: the many limitations pointed out in this study suggest that observational approaches conduct thorough sensitivity testing to different metrics, aggregations, and confounding variables before concluding for or against bias.
    * Experimental studies allow for more control and elimination of some confounding variables, like the conference. But still require careful evaluation of the question of "who is famous?" with several approaches and metrics. Perhaps a new approach can be tried where reviewers are actually asked to rank the "fame" of the authors they reviewed, but only after all papers have been reviewed, to avoid priming effects.
