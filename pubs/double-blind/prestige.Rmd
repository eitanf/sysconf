...
Figure \@ref(fig:metric-distribs) displays the distributions of the various author-level prestige metrics. All metrics are right skewed and there are reasons to suspect the quality of the values in both the upper and lower tails of these distributions. Because of name disambiguation issues, the upper tails are likely inflated.  In previous sub-sections we explored a few analysis techniques that dampen the impact of outliers, the Wilcoxon Signed Rank Test and a binary classification of fame, to see if the story changed. For a given metric and aggregation technique, the conclusions regarding prestige bias were similar between these outlier resistant measures and the other techniques.  Therefore, we chose not to winsorize these data, a common data cleaning technique for highly right skewed data.
...
```{r, echo = FALSE}
# Find prop missing by double_blind (by distinct authors??  What if they publish in both venues?)
ppl_blind <- ppl %>%
  left_join(all_confs, by = c("conf" = "key")) %>%
...
rep.all$metric <- factor(rep.all$metric, ordered = T,
                         ...
                         labels = c("No Imputation", "Flat Imputation", "Linear Imputation", "No Imputation", "Flat Imputation", "Linear Imputation"))
rep.all$aggregation <- factor(rep.all$aggregation, ordered = T,
                              levels = c("lead", "last", "median", "mean", "total", "max"),
                              labels = c("Lead author", "Last author", "Median", "Mean", "Total", "Maximum"))
rep.all$t_stat.t <- as.numeric(rep.all$t_stat.t)
