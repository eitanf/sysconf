...
cleaning_imputation <- function(dat, THRESH_RATIO, S2_THRESH){
  ...
  dat_train <- dat %>%
  filter(!imp, !is.na(s2npubs), s2npubs/npubs < 3) %>%
  group_by(name, gs_email) %>% #Since many authors are in the dataset several times
  summarize(npubs = first(npubs), s2npubs = min(first(s2npubs), S2_THRESH),
            hindex = first(hindex)) %>%
    ungroup()
...
To get a dataset ready for analysis, one must first wrangle or clean the data.  To clean the data requires the researcher to spend time exploring and interrogating their data to determine the quality of each variable and observation. This key, and time-consuming, step does not follow a formal set of rules but instead usually requires the researcher to make several subjective choices, which can impact the final results.  Additionally, the cleaning step and analysis step are typically not disjoint.  A researcher will often go back and perform additional cleaning as new issues crop up during analysis.  


Notes for self of things to include:

* Data cleaning is a large umbrella term for all the wrangling choices we make to go from raw data to analysis-ready data.
* A few key data cleaning actions:
    + Exploring the variation within each variable and the relationships between the variables
    + Detecting suspicious values and outliers
    + Identifying, and potentially imputing, missing or unusual values
        + Determining if the missing values are missing at random.
    + Removing erroneous observations or observations with missing, non-imputed values for key variables
    + Transforming variables to better suit the analysis needs (i.e., converting a numerical variable to categorical)
* Use graphs, summary statistics, and direct inspection of the raw data during the interrogation 

Need to discuss decisions we made:

* Use GS instead of S2 to find the total prior publications for each author.
    + Imputed values that were missing or very different from S2 
        + Arbitrary choice: Threshold for discrepancy between GS and S2
        + Choice: To use a linear model to predict GS for the imputed values where the predictor was S2
        + Choice: Truncated imputed values at 1000 (check)
* (For one version of the dataset): Removed rows that were missing GS.
* Used h-index as another prestige metric
    + Choice: To use a linear model to predict h-index for the imputed values where the predictor was S2
        + Did we truncate?
  
Need to discuss final wrangled datasets and differences between them.

* For each, discuss the assumptions that need to be made (or are likely violated) if we want to consider that set of observations as a representative sample.


```{r metric-distribs, echo=F, cache=T, warning=F, message=F, fig.cap="Density plots of all reputation proxy metrics (log-scale)."}
metrics.summary <- ppl %>%
...
total_npubs = paste0("n = ", nrow(filter(authors, !is.na(hindex))), sep = "")
...
corrplot(cor(cors, use="pairw"), type = "upper", order = "hclust", 
 ...
 * The correlation between npubs and s2npubs is much higher for ppl.filtered, probably because we removed the points where they disagree by a lot.
 ...
 Problems with GS data: missing authors; profiles that appear to aggregate more than one person; and multiple profiles for the same name. We dropped the first two categories.

 Most of the missing GS data is for inexperienced authors (prove with S2 data). Imputing this data won't make a big difference on prestige bias analysis, because presumably inexperienced authors haven't accrued much prestige yet.
 Nevertheless, we verified this by imputing h-index and citation data with linear models, using S2 data and papers-in-set as predictors. The inclusion of the imputed data did not meteriallly affect any of our conclusions in this paper (verify!).
...
<!-- Also holds when we impute GS (`r report_db(experience.best, "max", "npubs")`) -->
...
tbl %>%
  ...
  ggplot(aes(x = thresh, y = ratio, color = db_status)) +
    ...
    scale_color_discrete(name = "Blindess", labels = c("Double", "Single"))
```
...
tbl %>%
  ...
  ggplot(aes(x = thresh, y = ratio, color = db_status)) +
    ...
    ylab("Proportion of papers") +
    scale_color_discrete(name = "Review policy", labels = c("Double", "Single"))
```
