...
compute_glm <- function(exper = experience.all, metric = "npubs")
{
  col <- paste0("max_", metric)
  model <- glm(data = exper, as.formula(paste(col, "~ !double_blind")))
  linear_coef <- model$coefficients[2]
  linear_pval <- coef(summary(model))[2,4]

  col <- paste0("top_", metric)
  model <- glm(data = exper, as.formula(paste(col, "~ !double_blind")), family = "binomial")
  logit_coef <- model$coefficients[2]
  logit_pval <- coef(summary(model))[2,4]
...
Typically, systems papers list the primary contributor in the first author position, while the head of the lab is often listed last.
(In our dataset, only
...
---
Figure: heatmap, columns for metrics, rows for control variable, starting with None.
Number is coefficient of DB in glm for reputation; color is sign/magnitude of coefficient. Starts for p value.

Show heatmap of weight of double blind + p value for linear regression.
