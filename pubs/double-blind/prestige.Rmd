...
Across these three strategies, the sign of the relationship between review policy and the prestige metric stays the same for most aggregations, though the signficance of the results does vary (Fig. \@ref(fig:imp-results)).  The linear imputation methods acts like a compromise by typically providing a test statistic and p-value that is between those provided by the other two methods. Generally, it appears that imputing the missing data doesn't have a big impact on prestige bias analysis because most of the missing data are for the less experienced authors, who don't impact many of our aggregation metrics.  However, authors should be transparent about how they handle missing values.
...
ggplot(aes(y = npubs, x = s2npubs)) +
  ...
  guides(colour = FALSE) +
  labs(x = "Publications (SS)", y = "Publications (GS)") +
