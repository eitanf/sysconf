...
The simplest approach involves removing the missing values when computing aggregate measures.
...
We started with a simple model for prestige bias and then one by one we analyze the effect of each assumptions underlying the model: ...
...
Our suggestion for similar analyses on prestige bias is to try to analyze and present more than one or two such perspectives and note any disagreements.
...
-- Recommendation: take "quantity vs. quality" property into account when evaluating author prestige. Do it either by stratifying authors into classes, or including mean_citations_per_paper as a confounding variable in any regression, both for max h-index and max npubs. Is that even kosher to do, given that they're collinear?
...
In practice, various practical and ethical challenges mean that such trials are often unfeasible for evaluating bias, necessitating observational studies.
...
Experimental studies are hard to control and scale; observational studies can be very complex. 
