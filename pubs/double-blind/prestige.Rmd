...
<!-------------------------------------------------------------------------------------------->
## Handling Missing and Suspect Observations {#sec:cleaning}


To prepare the data for analysis required a thorough exploration of each variable and observations for missing or suspect values.  This *data cleaning* does not follow a formal set of rules but instead usually requires the researchers to make several subjective choices, each of which potentially impacts the final results. In this sub-section, we explore how the handling of missing and suspect observations potentially impacts conclusions of prestige bias.  

Figure \@ref(fig:metric-distribs) displays the distributions of the various author-level of prestige. All metrics are fairly right skewed and there are reasons to suspect the quality of the values in both the upper and lower tails of these distributions. Because of name disambiguation issues, the upper tails are likely inflations of their true values for many authors.  In previous sub-sections we explored a few analysis techniques that dampen the impact of outliers, the Wilcoxon Signed Rank Test and a binary classification of fame, to see if the story changed. For a given metric and XXX, the conclusions regarding prestige bias were similar between these outlier resistant measures and the other techniques.  Therefore, we chose not to winsorize these data, a common data cleaning technique for highly right skewed data.
