...
compute_experience <- function(who)
{
  who %>%
    ...
    summarize(conf = as.character(first(conf)), 
              coauthors = n(),
              max_papers = max(as_author), mean_papers = mean(as_author), median_papers = median(as_author), total_papers = sum(as_author), first_papers = first(as_author),
              max_h = max(hindex), mean_h = mean(hindex), median_h = median(hindex), total_h = sum(hindex), first_h = first(hindex),
              max_i10 = max(i10index), mean_i10 = mean(i10index), median_i10 = median(i10index), total_i10 = sum(i10index), first_i10 = first(i10index),
              max_citedby = max(citedby), mean_citedby = mean(citedby), median_citedby = median(citedby), total_citedby = sum(citedby), first_citedby = first(citedby),
              max_cites_per_paper = max(paper_cites), mean_cites_per_paper = mean(paper_cites), median_cites_per_paper = median(paper_cites), total_cites_per_paper = sum(paper_cites), first_cites_per_paper = first(paper_cites),
              max_pcs = max(as_pc), mean_pcs = mean(as_pc), median_pcs = median(as_pc), total_pcs = sum(as_pc), first_pcs = first(as_pc),
              max_npubs = max(npubs), mean_npubs = mean(npubs), median_npubs = median(npubs), total_npubs = sum(npubs), first_npubs = first(npubs),
              max_s2npubs = max(s2npubs), mean_s2npubs = mean(s2npubs), median_s2npubs = median(s2npubs), total_s2npubs = sum(s2npubs), first_s2npubs = first(s2npubs),
              top_university = max(top_university), top_company = max(top_company)
              ) %>%
    left_join(select(all_confs, c(key, double_blind)), by = c("conf" = "key"))
}

experience <- compute_experience(ppl.filtered)
...
{
  experience$over_h <- experience$max_h >= thresh
  c("db" = nrow(filter(experience, over_h, double_blind)),
    "sb" = nrow(filter(experience, over_h, !double_blind)),
    "p.value" = chisq.test(table(experience$over_h, experience$double_blind))$p.value
   )
}
...
{
  experience$over_npubs <- experience$max_npubs >= thresh
  c("db" = nrow(filter(experience, over_npubs, double_blind)),
    "sb" = nrow(filter(experience, over_npubs, !double_blind)),
    "p.value" = chisq.test(table(experience$over_npubs, experience$double_blind))$p.value
   )
}

########################
over_s2npubs <- function(thresh)
{
  experience.all$over_s2npubs <- experience.all$max_s2npubs >= thresh
  c("db" = nrow(filter(experience.all, over_s2npubs, double_blind)),
    "sb" = nrow(filter(experience.all, over_s2npubs, !double_blind)),
    "p.value" = chisq.test(table(experience.all$over_s2npubs, experience.all$double_blind))$p.value
   )
}
...
(`r report_db(experience, "max", "npubs")`).
...
(`r report_db(experience, "max", "papers")`).
...
(`r report_db(experience, "max", "cites_per_paper")`).
The h-index metric also is strongly indicated with higher values in DB conferences
(`r report_db(experience, "max", "h")`).

The total citations metric shows a much weaker, non-statistically significant association in the same direction
(`r report_db(experience, "max", "citedby")`),
while the i10 metric shows a non-statistically significant association in the opposite direction
(`r report_db(experience, "max", "i10")`).
...
(`r report_db(experience, "max", "pcs")`).
...
for (metric in c("npubs", "papers", "citedby", "cites_per_paper", "h", "i10")) {
  for (aggr in c("max", "total", "mean", "median")) {
    tbl <- rbind(tbl, compute_db(experience, aggr, metric))
  }
}

# Append lead-author stats
for (metric in c("s2npubs", "npubs", "papers", "citedby", "cites_per_paper", "h", "i10")) {
...
rep.all$metric <- factor(rep.all$metric, ordered = T,
                         levels = c("npubs", "s2npubs", "papers", "citedby", "cites_per_paper", "h", "i10"),
...
ggplot(rep.all, aes(x = metric, y = aggregation)) +
  ...
  scale_fill_gradient2(low = "orange", high = "blue", limits = c(-26, 26)) +
...
<!---------------------------------------------------->
### Binary classifications of fame
...
e_with_seniority <-
experience %>%
    mutate(Experience = as.factor(ifelse(max_h < 12, "Novice", ifelse(max_h <= 18, "Mid-career", "Experienced"))))
...
For this analysis, we follow the methodology of Tomkins, Zhang, and Heavlin who examined prestige bias in the 2017 Web Search and Data Mining (WSDM) conference [@tomkins17:reviewer].
...
(`r pct(nrow(filter(experience, top_university > 0, double_blind)), nrow(filter(experience, double_blind)))`% vs.
`r pct(nrow(filter(experience, top_university > 0, !double_blind)), nrow(filter(experience, !double_blind)))`%,
`r report_test(chisq.test(table(experience$top_university, experience$double_blind)))`)
The same can be said for top-company affiliation
(`r pct(nrow(filter(experience, top_company > 0, double_blind)), nrow(filter(experience, double_blind)))`% vs.
`r pct(nrow(filter(experience, top_company > 0, !double_blind)), nrow(filter(experience, !double_blind)))`%,
`r report_test(chisq.test(table(experience$top_company, experience$double_blind)))`).
