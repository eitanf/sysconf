...
experience.lead <- compute_experience(left_join(roles, persons) %>% filter(role == "lead_author") %>% mutate(paper_cites = citedby / npubs))
experience.last <- compute_experience(left_join(roles, persons) %>% filter(role == "last_author") %>% mutate(paper_cites = citedby / npubs))
experience.best <- compute_experience(ppl.best_source)
...
metrics.summary <- ppl %>%
  ...
  mutate(Metric = recode_factor(Metric,
                                ...
                                i10index = "i10-index", hindex5y = "h-index (5 years)", i10index5y = "i10-index (5 years)")) %>%
