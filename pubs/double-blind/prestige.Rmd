...
```{r median-diffs, echo = F, warning = F, fig.cap="Difference in medians for various reputation metrics and aggregations across papers using two-sided Wilcoxon test. Positive W-statistic values (cyan) indicate higher reputation in DB conferences. Negative values (pink) indicate higher reputation in SB conferences. Stars indicate p values below 0.05, 0.01, and 0.001, respectively."}
ggplot(rep.all, aes(x = metric, y = aggregation)) +
  geom_tile(aes(fill = w_stat.W)) +
  scale_fill_gradient2(low = "pink", high = "cyan") +
  geom_text(aes(label = paste0(round(w_stat.W, 2), format_p_value(w_pval, p_option = "stars")))) +
  ylab("Paper aggregation method") +
  xlab("Reputation metric") +
  labs(fill = "W statistic") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

```

(((I think this is a bad comparison though. Our data aren't paired, as assumed by wilcox.test. Should we use Mood's median test instead?
When looking at mean_sb and median_sb, switching to median sometimes changes the direction of the conclusions. Can we depict it?)))
