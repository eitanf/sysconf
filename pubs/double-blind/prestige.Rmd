...
To evaluate the effect of these confounding variables, we built fixed-effects and mixed-effects linear regression models with author prestige as the outcome variable (maximum in paper), an indicator variable for reviewing policy (where 1 = SB, 0 = DB) as the fixed effect predictor variable, and conference as a random effect in the mixed-effects model (Figs. \@ref(fig:linear-regression) and  \@ref(fig:linear-mixed-regression)).  The first row of Fig. \@ref(fig:linear-regression) is equivalent to the first row in Fig. \@ref(fig:mean-diffs) and shows the coefficient of the reviewing policy alone on author prestige, which often exhibits a statistically strong association. Each row afterwards includes one of the variables listed above as an additional fixed effect that could stand in for conference prestige.  All of the analyses conducted up the linear regression model assume the papers are independent observations.  However, we believe papers in the same conference are likely to be more correlated and therefore account for this correlation explicited in the linear mixed-model (Fig. \@ref(fig:linear-mixed-regression)).  In comparing Figs. \@ref(fig:linear-regression) and  \@ref(fig:linear-mixed-regression), notice that the significance of reviewing policy is greatly dampen when accounting for conference.  For the rest of this section, we will focus on the mixed-effects model. 
...
On the lower tail, note that `r round(mean(is.na(ppl$npubs)) * 100, digits = 2)`% of the values for the GS measures are missing for authors for who we could not uniquely identify a GS profile.  Proportionally, these authors are distributed almost evenly between SB and DB publications.
...
If we look at GS statistics for each paper exactly `r how_long` months after it were published, we find that DB papers average
