...
experience.all <- ppl %>%
    ...
    summarize(conf = as.character(first(conf)), 
              coauthors = n(),
              max_papers = max(as_author), mean_papers = mean(as_author), median_papers = median(as_author), total_papers = sum(as_author),
              max_pcs = max(as_pc), mean_pcs = mean(as_pc), median_pcs = median(as_pc), total_pcs = sum(as_pc),
              max_s2npubs = max(s2npubs), mean_s2npubs = mean(s2npubs), median_s2npubs = median(s2npubs), total_s2npubs = sum(s2npubs)) %>%
    left_join(select(all_confs, c(key, double_blind)), by = c("conf" = "key"))
...
experience <- ppl.filtered %>%
    ...
    summarize(conf = as.character(first(conf)), 
              coauthors = n(),
              max_papers = max(as_author), mean_papers = mean(as_author), median_papers = median(as_author), total_papers = sum(as_author),
              max_h = max(hindex), mean_h = mean(hindex), median_h = median(hindex), total_h = sum(hindex),
              max_i10 = max(i10index), mean_i10 = mean(i10index), median_i10 = median(i10index), total_i10 = sum(i10index),
              max_citedby = max(citedby), mean_citedby = mean(citedby), median_citedby = median(citedby), total_citedby = sum(citedby),
              max_cites_per_paper = max(citedby / npubs), mean_cites_per_paper = mean(citedby / npubs), median_cites_per_paper = median(citedby / npubs), total_cites_per_paper = sum(citedby / npubs),
              max_pcs = max(as_pc), mean_pcs = mean(as_pc), median_pcs = median(as_pc), total_pcs = sum(as_pc),
              max_npubs = max(npubs), mean_npubs = mean(npubs), median_npubs = median(npubs), total_npubs = sum(npubs),
              max_s2npubs = max(s2npubs), mean_s2npubs = mean(s2npubs), median_s2npubs = median(s2npubs), total_s2npubs = sum(s2npubs)) %>%
    left_join(select(all_confs, c(key, double_blind)), by = c("conf" = "key"))
...
compute_db <- function(exper = experience, aggr = "max", metric = "npubs")
{
  ...
  c(aggregation = aggr,
    metric = metric,
    mean_sb = round(mean(sb[[column]]), 2),
    mean_db = round(mean(db[[column]]), 2),
    median_sb = round(median(sb[[column]]), 2),
    median_db = round(median(db[[column]]), 2),
...
using with s2npubs also avoids the problem of the missing data on the left end.
...
Then, switch to medians and wilcox.test, same table and show how differences disappear, like in [@tung06:impact].
