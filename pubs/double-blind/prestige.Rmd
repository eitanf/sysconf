...
We found for example that different reputation metrics can yield contradictory results.
Even the same popular metric, number of publications, can yield very different distributions based on the database it comes from, because each database comes up with its own choices to inherently hard questions like "what constitutes a publication" and "how to disambiguate names".
...
Some researchers clearly appear to publish a smaller number of highly competitive papers, some prefer to publish prolifically in less competitive venus, and others lie in the spectrum between these two groups.
...
Our main recommendation for future prestige bias research is thefore to not focus on a single reputation metric.
