...
The gender gap in computer science (CS) is a well-known problem with significant societal effects, such as inequality in economic opportunities for women and an undersupply of researchers engineers in the rapidly growing discipline \cite{editorials18:science,mattis07:upstream}.
...
Although some recent indications show these numbers could be growing, they remain alarmingly low, and the rate of growth remains discouragingly slow \cite{wang19:trends}.

CS is a large and diverse discipline with different characteristics in each of its constituent fields.
...
We take a particularly closer look at the United States, which currently dominates the field, both in terms of affiliated researchers and of hosted conferences.
 
There exists sporadic evidence of an acute gender gap in specific subareas of systems \cite{destefano18:micro, jerger17:gender, mattauch20:bibliometric}, but we were unable to find a systematic examination of women's underrepresentation in the entire field.
...
This approach has been previously tested for different researcher populations \cite{elsevier17:gender,lariviere13:bibliometrics,mattauch20:bibliometric}.
...
The conferences we selected include some of the most prestigious systems conferences (based on indirect measurements such as Google Scholar's metrics), as well as several smaller or less-competitive conferences for contrast (Table \@ref(tab:sys-confs)).[^It should be noted that venue prestige and gender-gap are not independent. For example, men tend to self-cite more than women \cite{king17:men}, prestigious mathemtics journals underrepresent women~\cite{mihaljevic20:authorship}, and novel research published by women is less likely to be impactful \cite{hofstra20:diversity}. We do not address this gap within the scope of this paper, but note that we found no direct correlations between a conference prestige metrics and its ratio of female authors.]
...
Because our metric for the gender gap counts the percentage of women among authors, we collected the names of all
...
Papers in systems tend to list the primary contributor in the leading (first) position, so we examined the gender of first authors as well.
...
This method provided more gender data and higher accuracy than automated approaches based on first name and country, especially for women \cite{karimi16:gender, lariviere13:bibliometrics, mattauch20:bibliometric, squazzoni20:noevidence, wang19:trends}.
...
We found no disagreements for these authors, which suggests that the likelygood of disagreements among the remaining authors is low.
...
For each author and PC member, we collected all metrics in their GS profile, such as total previous publications (ca. 2017), H-index, etc.^[See README.md in the data/authors directory of the data repository for complete details.]
We also looked up each author's affiliation institute on GS to find their current country of residence and research sector whenever they could be clearly inferred using hand-coded regular expressions.
...
From authors' affiliations, we broadly categorized their sector as either "COM" for industry
...
Finally, from the conferences web site and proceedings we collected information on any explicit policies the conference made to increase attendance diversity (Table \@ref(tab:diversity-policy)), so that we could measure their effects, if any, on the gender gap.


For statistical testing, group means were compared pairwise using $\chi^{2}$ test; differences between distributions of two categorical variables were tested with Welch's two-sample t-test; and comparisons between numeric properties of two groups were evaluated with Pearson's product-moment correlation. All statistical tests are reported with their p-values.
...
We believe and demonstrate the magnitude of this undercounting to be small and insufficient on its own to explain the large gap with the overall CS statistics from past publications (which also use the same metric, with the same limitations).

In the literature, we found few controlled experiments that evaluate the peer-review process on both accepted and rejected papers, and they are typically limited in scope to one conference or journal \cite{parno17:SPsurvey, shah18:design, tomkins17:reviewer}.
...
The main limitation of this approach is that it may miscount women if there is significant gender bias in the review process.
There are systemic effects that cause this undercounting of women and this limitation.
Homegeneity in journals, conference culture
\cite{???}
With that in mind, we are evaluating the system as it was intended, directly comparing our findings to other studies who employed the same counting approach.
...
Even with manual gender assignment, `r pct(nongend, nrow(persons), 2)`% of researchers still have unknown gender.
...
We first artificially set the gender of all `r nongend` researchers to female, and then to male, and recomputed all statistical analyses.
...
With the data we collected on conference participants, we can compute the ratio of women in different conference roles: peer-reviewed authors, reviewers, and invited presentations (Table \@ref(tab:pct-by-role)).
We found that approximately one in ten published authors were women, about $\sfrac{1}{3}$ to $\sfrac{1}{2}$ of the ratio in the rest of CS.
...
of authors are named in more than one paper, we compared results of counting each person exactly once and counting repeated occurrences of each person.
...
knitr::kable(role_genders[c(6,7,4,5,8,1,2,3),], align=c("lrrrr"), booktabs = T, row.names = F,
             caption = "Researcher count and ratio of women by conference role for people with assigned genders. Researchers are either aggregated with repeats, counting multiple appearances in role, or identified uniquely per role. Only the author roles are peer reviewed, where typically lead authors in systems are the primary contributors and last authors are the senior member of the team.")
```


The second-largest group of researchers, and the largest invited group, is that of PC members.
This group also indirectly affects the representation of women among published authors, because PC members, through their reviews, decide which papers get published.
The ratio of female PC members (FPR) is significantly higher than the ratio of female authors, 
(`r filter(role_genders, Role=="PC member")$Women` vs. `r filter(role_genders, Role=="Author")$Women`, `r report_stat(p_option = "only", chisq.test(gender_table))`),
which raises the question: which of the two ratios is more representative of women's true participation rate in Systems research?

The typical bibliometric approach to estimate participation by gender in a field of research is to look at published authors \cite{cohoon11:cspapers,mattauch20:bibliometric}.
This metric is not always accurate.
It ignores researchers who cannot publish, and potentially undercounts women scientists because they tend to publish less than men in many fields \cite{elsevier20:journey,ghiasi15:compliance,lariviere13:bibliometrics,symonds06:gender}, possibly owing to a higher service load \cite{guarino17:faculty,misra12:gender,omeara17:asked}.
Indeed, women authors published only
`r round(mean(filter(authors, gender == "F")$as_author, na.rm = T), 2)` 
papers in our dataset on average, compared to males'
`r round(mean(filter(authors, gender == "M")$as_author, na.rm = T), 2)`
(`r report_stat(p_option = "only", t.test(filter(authors, gender == "F")$as_author, filter(authors, gender == "M")$as_author), 2)`).
...
Compared to PC members, authors typically undergo blind and competitive peer review (averaging an acceptance rate of
...
If the process allowed for a disproportionate number of women-authored papers to be published, it would mean that the gender gap in the author sample is not reflective of the researcher population as a whole, but that is not what we found.
Mirroring studies from other fields that found no evidence of gender bias in the peer-review process \cite{cohoon11:cspapers,fox16:gender,squazzoni20:noevidence}, we found that women's papers are actually accepted at slightly higher rates when their identity is known to reviewers
(`r sum(all_confs$double_blind)` single-blind conferences)
or when it is prominent in the first author position
...
Our author survey also found that the reviews women received in the single-blind conferences in our set showed  similar or higher grades than men's \cite{frachtenberg20:survey}.
...
This hypothesis, and the reliance of FAR instead of FPR, is supported by three observations.
...
First, if chairs are indeed oversampling women for PC roles, we would expect to see differences in various statistics for male and female PC members.
For example, chairs may have to search deeper in the researcher pool to recruit women to the PC, leading to lower research experience among female PC members, compared to their male counterparts.
We find corroboration for this prediction in our data (Fig. \@ref(fig:bibliometrics)).
For example, the mean H-index of the female PC member in our dataset,
`r round(mean(filter(demographics, role == "pc", gender == "F")$hindex, na.rm = T), 2)`,
is significantly lower than the males'
`r round(mean(filter(demographics, role == "pc", gender == "M")$hindex, na.rm = T), 2)`
(`r report_stat(p_option = "only", gendiff("pc", "H-index"), 3)`),
...
`r report_stat(p_option = "only", gendiff("author", "H-index"), 3)`).
...
mean PC roles (member and chair) per woman, compared to
`r mpc <- filter(pcs, gender == "M") %>% summarize(mean(as_pc + as_pc_chair)); round(mpc, 2)`
per man
(`r report_stat(p_option = "only", t.test(filter(pcs, gender == "F")$tot_pc, filter(pcs, gender == "M")$tot_pc), 2)`).
It is conceivable that the additional time committed to PC service explains some of the reduced publication rate we observed earlier among women.
However, authors who serve as PC members also tend to publish more papers
(Pearson's `r report_stat(cor.test(authors$as_pc, authors$as_author))`).
The weak but positive association between number of committees and number of papers suggests that a relative overrepresentation of women in PCs is not commensurate with underrepresentation among authors.
...
Therefore, conferences with atypical gender gaps introduce more variance to PC gender ratios than to those of authors.
...
Excluding this pair reduces the mean FPR across the remaining conferences by
...
Conversely, removing the two conferences with the lowest female author ratios (HotI and VEE) only bumps up the mean FAR by
...
Skewness in distribution therefore pulls the mean female ratios higher among PCs than it pulls it lower among authors, reaffirming our assertion that the percentage of women authors in our dataset is a more reliable indicator of the overall gender gap in the field.


# Explanatory variables

The next step in understanding the gender gap is to look at the factors that may be associated with it.

## Conference factors

Female author representation can vary considerably from one conference to the next, from a minimum of
`r round(100 * min(conf_gender$author), 2)`%
to a maximum of
`r round(100 * max(conf_gender$author), 2)`%
(mean: `r round(100 * mean(conf_gender$author), 2)`%,
SD: `r round(100 * sd(conf_gender$author), 2)`%).
Examining the differences between conferences could offer clues as to which factors could affect the gender gap.



```{r women-rep-author-vs-PC, message = F, echo = F, cache = F, warning = F, fig.cap="Underrepresentation of women among authors (FAR) by conference, compared to conference size (from Table 1), double-blind reviewing, and women PC representation. None of these factors is significantly associated with FAR. Density plots on the axes show the relative distribution of female PC members and authors for single- and double-blind reviews."}
...
Averaging the ratio of women by conferences, as opposed to by authors or papers (both computed in Table \@ref(tab:pct-by-role)), can produce different results, because smaller conferences receive the same weight as conferences with many more authors and papers.
...
Further, there appears to be no correlation between the number of papers in a conference and its female author ratio
(`r report_stat(cor.test(conf_gender$author, conf_gender$npapers))`).

Earlier studies have found evidence of gender bias in the peer-review process, especially in single-blind reviews, although more recent surveys are inconclusive \cite{lee13:bias,mcgillivray18:uptake,squazzoni20:noevidence}.
Fig. \@ref(fig:women-rep-author-vs-PC) also looks for a possible link between double-blind reviewing and increased FARs.
In our dataset, conferences with double-blind reviewing actually exhibit a lower FAR
(`r round(100 * mean(filter(conf_gender, double_blind)$author), 2)`% average vs. 
`r round(mean(100 * filter(conf_gender, !double_blind)$author), 2)`% for single-blind conferences,
`r report_stat(p_option = "only", t.test(filter(conf_gender, double_blind)$author, filter(conf_gender, !double_blind)$author))`).

Another conference policy often employed to increase participant diversity is to invite a more diverse reviewer body.
...
Nevertheless, we found no correlation between higher female PC ratios and higher FARs
(`r report_stat(cor.test(conf_gender$PC, conf_gender$author))`).
We also looked at the other visible conference roles: keynote speakers, session chairs, and panelists.
These roles offer additional indication of the field's diversity, and may play a role in the diversity of conference attendees \cite{destefano18:micro,davenport14:studying,ford18:gender,le20:iscb}.
However, the Pearson's correlations between FAR and these roles do not reveal such a relationship in our dataset
(`r report_stat(cor.test(conf_gender$keynote, conf_gender$author))`; `r report_stat(cor.test(conf_gender$session, conf_gender$author))`; and `r report_stat(cor.test(conf_gender$panel, conf_gender$author))`, respectively).
...
In addition to these factors, we collected and examined various other conference factors, including: conference history (age, number of past papers, and past citations); prestige (acceptance rate, citations per paper, H5-index); and size (number of submissions, authors, coauthors per paper, and PC members).
All factors appear to be nonsignificant for FAR.

<!---
...Edit out...: Conference factors with no effect:

  * age in years: `r report_stat(cor.test(conf_gender$author, conf_gender$age))`
  * past papers: `r report_stat(cor.test(conf_gender$author, conf_gender$past_papers))`
  * past citations: `r report_stat(cor.test(conf_gender$author, conf_gender$past_citations))`
  * mean historical length: `r report_stat(cor.test(conf_gender$author, conf_gender$mean_historical_length))`
  * mean historical citations: `r report_stat(cor.test(conf_gender$author, conf_gender$mean_historical_citations))`
  * h5 index / median: `r report_stat(cor.test(conf_gender$author, conf_gender$h5_index))` ; `r report_stat(cor.test(conf_gender$author, conf_gender$h5_median))`
  * pc size:  `r report_stat(cor.test(conf_gender$author, conf_gender$pc_size))`
  * no. of submissions:  `r report_stat(cor.test(conf_gender$author, conf_gender$submissions, use = "pairwise.complete.obs"))`
  * authors num:  `r report_stat(cor.test(conf_gender$author, conf_gender$authors_num))`
  * mean authors per paper:  `r report_stat(cor.test(conf_gender$author, conf_gender$mean_authors_per_paper))`
  * pc author ratio:  `r report_stat(cor.test(conf_gender$author, conf_gender$pc_author_ratio))`
  * pc paper ratio:  `r report_stat(cor.test(conf_gender$author, conf_gender$pc_paper_ratio))`
  * acceptance rate:  `r report_stat(cor.test(conf_gender$author, conf_gender$acceptance_rate, use = "pairwise.complete.obs"))`
  * mean review load:  `r report_stat(cor.test(conf_gender$author, conf_gender$mean_review_load, use = "pairwise.complete.obs"))`
  * chairs num: `r cn <- 1; report_stat(t.test(filter(conf_gender, chairs_num <= cn)$author, filter(conf_gender, chairs_num > cn)$author))`
  * open access: `r report_stat(t.test(filter(conf_gender, open_access)$author, filter(conf_gender, !open_access)$author))`
* Sponsoring organization: `r report_stat(t.test(filter(conf_gender, is_org_IEEE)$author, filter(conf_gender, !is_org_IEEE)$author))`; `r report_stat(t.test(filter(conf_gender, is_org_ACM)$author, filter(conf_gender, !is_org_ACM)$author))`; `r report_stat(t.test(filter(conf_gender, is_org_USENIX)$author, filter(conf_gender, !is_org_USENIX)$author))`.
  * month_of_year: `r summary(aov(author ~ month_of_year, data=conf_gender))`
...
Several policies have been proposed to increase diversity and women's participation in scientific conferences include: a designated inclusivity chair; a code of conduct or anti-harassment policy; special events and meetings to promote diversity; assistance with childcare during the conference; travel grants for underrepresented populations; and the collection and dissemination of diversity data \cite{collins16:diversity,gould18:conferences,martin14:ten}.
In our dataset, `r sum(all_confs$diversity_effort)` of the `r nrow(all_confs)` implemented at least one of these suggestions (Table \@ref(tab:diversity-policy)), but that did not ostensibly lead to higher FARs
...
`r report_stat(p_option = "only", t.test(filter(conf_gender, diversity_effort)$author, filter(conf_gender, !diversity_effort)$author), 2)`).
For example, the only two conferences with an inclusivity chair ranked among the lowest conferences for FAR.

It is possible that these conference initiatives were in fact more reactive than proactive, in an attempt to improve previous statistics.
...
Throughout this period, women's attendance rate remained near constant at around 13%-14% (author demographics are only shared for 2018, with 12% FAR).
ISC is another large conference that, like SC, employs various inclusivity initiatives, including naming a dedicated diversity chair and reporting attendee demographics.
It does not report FAR, but we have manually computed FAR in the range 5%--9% for the years 2017--2020, lower than the average conference in our dataset.
...
It is plausible that inclusivity initiatives are only one of the selection criteria when choosing a conference to publish in, and other criteria such as conference date, location, prestige, and subfield take precedence. 
For example, among the four computer architecture conferences in our set (ASPLOS, HPCA, ISCA, MICRO), all with similar acceptance rates, only ISCA offers any diversity initiative, but all four show similar FAR.
...
To summarize, inviting more women to visible conference role and implementing diversity-focused policies likely contributes to more inclusive conferences, but is insufficient on its own to spontaneously add women authors to the field.
...
Aside from conference-related factors, we also examined the effects on FAR of three author-related factors: research experience, work sector, and country of affiliation.
...
As the previous section discusses, female PC members exhibit a significantly lower mean H-index than males, but there are no statistically significant differences in H-index among authors of each gender.
In fact, the 1st-, 2nd-, and 3rd-quartile statistics are also very similar across genders.
Other experience metrics, such as the total number of an author's prior publications, show comparable similarity across gender.
The only gender difference shown in Fig. \@ref(fig:bibliometrics) is in the tail of the author distribution, with men composing the majority of the top percentile.
...
It is neverthless consistent with data in Table \@ref(tab:pct-by-role), where women in last author position (typically representing the senior member of the team), appear at a lower rate than overall women authors, and especially lead authors (typically representing a junior member of the team).
It also agrees with past observations that women continue to senior academic ranks at a lower rate than men \cite{elsevier20:journey,fox06:engineering,frantzana19:women,gerhard07:undergraduate,mattis07:upstream}.


\newpage{}
### Work sector

Contrary to the gender gap in research experience, the gender gap in work sector is more prominent for authors that it is for PC members.
Most of the unique authors in our dataset are affiliated with academic institutes
(`r pct(sum(da$sector == "EDU"), nrow(da), 1)`%),
followed by industry
(`r pct(sum(da$sector == "COM"), nrow(da), 1)`%)
and government
(`r pct(sum(da$sector == "GOV"), nrow(da), 1)`%).
The respective FARs for each sector,
`r pct(nrow(filter(da, sector == "EDU", gender == "F")), sum(da$sector == "EDU"))`%,
`r pct(nrow(filter(da, sector == "COM", gender == "F")), sum(da$sector == "COM"))`%, and
`r pct(nrow(filter(da, sector == "GOV", gender == "F")), sum(da$sector == "GOV"))`%,
show women to be significantly underrepresented in industry, compared to academia
(`r report_stat(p_option = "only", chisq.test(table(da$sector, da$gender)[-3,-3]))`).
Other studies have also found relatively fewer women engineers in industry research positions \cite{fox06:engineering,ghiasi15:compliance,tao16:employment}.


<!---
The statistics for repeated authors are similar ($p>0.05$), but are very sensitive to the long-tailed distribution of papers per author.
Of the top 20 most prolific authors in each sector, 19 are men, lower the FARs for academia and government to
`r pct(nrow(filter(ra, sector == "EDU", gender == "F")), sum(ra$sector == "EDU"))`% and
`r pct(nrow(filter(ra, sector == "GOV", gender == "F")), sum(ra$sector == "GOV"))`%.
However, two of the most prolific women also shared industry affiliation during 2017, which increases industry's FAR to
`r pct(nrow(filter(ra, sector == "COM", gender == "F")), sum(ra$sector == "COM"))`% and erases further the differences between the sectors
(`r report_stat(chisq.test(table(ra$sector, ra$gender)[,-3]))`).


But these statistics do not reveal the full picture.
Although the distribution across sectors for unique authors is similar to that of repeated authors ($p>0.05$), that is not the case for representation of women.
Women represent only
`r  wa <- filter(da, gender == "F"); wacom <- filter(wa, sector == "COM"); pct(nrow(wacom), nrow(filter(da, sector == "COM")), 2)`%
of the unique authors from industry, compared to
`r wanocom <- filter(wa, sector != "COM"); pct(nrow(wanocom), nrow(filter(da, sector != "COM")), 2)`%
in government and academia combined
(`r report_stat(p_option = "only", chisq.test(data.frame(w = c(nrow(wacom), nrow(wanocom)), m = c(nrow(filter(da, gender == "M", sector == "COM")), nrow(filter(da, gender == "M", sector != "COM"))))))`).
This result agrees with past studies that found relatively fewer women engineers in industry research positions \cite{fox06:engineering,ghiasi15:compliance,tao16:employment}, but contradicts the overall FAR statistics for repeated authors, which are often used in comparable bibliometric studies \cite{wang19:trends}.

This contradiction could be explained by differences in author productivity across sectors and genders.
Women in industry authored relatively more papers than men in our dataset (averaging
`r round(mean(filter(ra, gender == "F", sector == "COM")$as_author), 2)
round(mean(filter(ra, gender == "F", sector == "COM")$as_author), 2)
`
papers per author, vs. men's 
`r round(mean(filter(ra, gender == "M", sector == "COM")$as_author), 2)`,
`r report_stat(t.test(p_option = "only", filter(ra, gender == "F", sector == "COM")$as_author, filter(ra, gender == "M", sector == "COM")$as_author))`).
A partial explanation to this sector difference arises from the long tails of productivity in our dataset.
Only one of the top twenty most-prolific academic authors is a woman, accounting for
`r top20edu <- filter(da, sector == "EDU") %>% arrange(desc(as_author)) %>% head(20); pct(sum(filter(top20edu, gender == "F")$as_author), sum(top20edu$as_author))`% 
of their collective paper output.
The same is true for government-affiliated authors.
In industry, however,
`r top20com <- filter(da, sector == "COM") %>% arrange(desc(as_author)) %>% head(20); sum(top20com$gender == "F")`
of the top 20 authors are women, accounting for
`r pct(sum(filter(top20com, gender == "F")$as_author), sum(top20com$as_author), 0)`%
of the papers.

These two gender effects cancel each other out when comparing statistics for repeated authors---as many past studies do---to reveal only nonsignificant gender differences in the gender gap across sectors.
(`r report_stat(p_option = "only", chisq.test(data.frame(w = c(nrow(filter(ra, gender == "F", sector == "COM")), nrow(filter(ra, gender == "F", sector != "COM"))), m = c(nrow(filter(ra, gender == "M", sector == "COM")), nrow(filter(ra, gender == "M", sector != "COM"))))))`).
-->

The distribution of work sectors among the unique PC members appears similar, with
`r pct(sum(dp$sector == "EDU"), nrow(dp), 1)`% hailing from academia,
`r pct(sum(dp$sector == "COM"), nrow(dp), 1)`% from industry, and
`r pct(sum(dp$sector == "GOV"), nrow(dp), 1)`% from government.
This similarity suggests that no sector is particularly favored over the others in program committees.
As discussed earlier, female representation is higher for PCs than for authors, but curiously, not by the same magnitude.
...
country_stats <- function(code_, who_, by_affil = TRUE)
{
  ...
  return(paste0(pct(tbl[2, 2], sum(tbl[2,], 2)), "%", report_stat(chisq.test(tbl), p_option = "stars")))
}
...
country_count = persons %>% drop_na(country) %>% group_by(country) %>% summarize(n = n()) %>% nrow()

all_others <- data.frame(code = "---",
                         authors = sum(other_ppl$as_author),
                         pcs = sum(other_ppl$as_pc),
                         a_by_affil = paste0(pct(sum(filter(other_ppl, gender == "F")$as_author), sum(other_ppl$as_author)), "%"),
                         a_by_conf = NA,
                         pc_by_affil = paste0(pct(sum(filter(other_ppl, gender == "F")$as_pc), sum(other_ppl$as_pc)), "%"),
                         pc_by_conf = NA,
...
#    column_spec(column = 2:4, width = "2cm") %>%
  add_header_above(c("Country", "Conferences", "Authors", "FAR affiliated", "FAR hosted", "PCs", "FPR affiliated", "FPR hosted")) %>%
...
(`r report_stat(cor.test(cntry$authors, parse_number(cntry$a_by_affil)))`).
This correlation is even weaker if we omit the outlier US, which comprises most of the affiliated authors
...
with gender and country information.
...
`r report_stat(p_option = "only", chisq.test(data.frame(c(sum(us_a$gender == "F"), sum(us_a$gender == "M")), c(sum(not_us_a$gender == "F"), sum(not_us_a$gender == "M")))), 3)`).
Half or so of the US-based computer scientists in CS (and in our dataset) are likely foreign-born \cite{frachtenberg20:survey,national20:science}, but this distinction does not appear to explain differences in the gender gap \cite{frachtenberg20:survey,goyette99:intersection,hango13:gender,tong10:place}.

Another possible explanation is that as the host of most systems conferences in our dataset, the US might be more appealing to researchers who prefer domestic travel, such as parents of young children.
In all countries except South Korea and Italy, we found significantly higher representation of local-affiliated authors.
...
However, we found no evidence for a gender difference in this preference---not in the US, where there are actually fewer women in US-hosted conferences---and not even more generally, since the correlation between a country's FAR by affiliation and by hosted conference is nonexistent
(`r report_stat(cor.test(parse_number(cntry$a_by_conf), parse_number(cntry$a_by_affil)))`).
...
The number of authors affiliated with a country is highly correlated with the number of PC members
(`r report_stat(cor.test(cntry$authors, cntry$pcs))`),
which in turn means most PC members hail from the West.
It is encouraging to note, however, that Western reviewers are not significantly overrepresented compared to authors, as has been observed in journals in other fields \cite{publons18:peer}.

For PC members, the gender-gap differences are even higher than for authors, with women representing
...
(`r report_stat(p_option = "only", chisq.test(data.frame(c(sum(us_p$gender == "F"), sum(us_p$gender == "M")), c(sum(not_us_p$gender == "F"), sum(not_us_p$gender == "M")))), 3)`).
...
As discussed previously, overall FPR is almost double that of overall FAR. 
With the exception of a few countries, primarily near southeast Asia, most individual countries also exhibit a higher FPR than FAR, which may be related to the subjective selection process of PC roles.
Moreover, with the exception of the US and Spain, all countries exhibit an even higher FPR for hosted conferences, unlike FARs.

It is worth noting that for researchers of unknown country affiliation, both FAR and FPR are very similar to the overall statistics, which suggests there was limited or no selection bias based on availability of country and gender information.
...
The main finding of this study is that only 10.5% of systems authors are women, a ratio that is significantly lower than the rest of CS.
...
The data shows clearer gender-gap variations by country of affiliation, but these cannot be fully explained by geographical region, economic development, or gender gap index.

The gender gap therefore appears to be endemic to the field, which raises the question, what causes the particular low representation of women in systems, compared to the rest of CS?
We plan to examine various other gender-imbalance factors that have been identified as systemic to CS, and evaluate whether they have a particularly pronounced effet on the systems field or any of its subfields.
Such factors include the early-childhood gender gap in programming, departmental and workplace culture, and the shortage of female peers and mentors and the perception of low societal impact of CS careers.
