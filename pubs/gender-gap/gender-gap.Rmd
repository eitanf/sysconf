...
The main limitation of this approach is that it may miscount women if there is significant gender bias in the publication or review process.
...
We found that approximately 10% of published authors were women, about $\sfrac{1}{3}$ to $\sfrac{1}{2}$ of the ratio in the rest of CS.
...
knitr::kable(role_genders[c(6,7,4,5,8,1,2,3),], align=c("lrrrr"), booktabs = T, row.names = F,
             caption = "Researcher count and ratio of women by conference role. Researchers are either aggregated by total appearances or identified uniquely, once per role. Only authors are selected by peer reviewed. Typically, lead authors in systems are the primary contributor and last authors are the senior member of the team.")
```
...
Earlier studies have found evidence of gender bias in the peer-review process, especially in single-blind reviews, although more recent surveys are inconclusive \cite{lee13:bias,mcgillivray18:uptake,squazzoni20:noevidence,squazzoni21:peer}.
...
We have already observed that women PC members exhibit a significantly lower mean H-index than men, but that for authors, the difference in gender means is nonsignificant.
Comparing the number of past publications of authors as another proxy metric for experience again reveals a nonsignificant difference in means
(`r report_stat(t.test(filter(demographics, role == "author", gender == "F")$s2npubs, filter(demographics, role == "author", gender == "F")$s2npubs), df = T)`).
The 1st-, 2nd-, and 3rd-quartile statistics are also very similar across genders.
...
(`r top1 <- filter(demographics, role == "author", !is.na(hindex)) %>% slice_max(prop = 0.01, order_by = hindex); pct(sum(top1$gender == "F"), nrow(top1), 2)`%).
...
Contrary to the gender gap in research experience, the gender gap in work sector is more prominent for authors than it is for PC members.
...
As discussed earlier, female representation is higher for PCs than for authors, but notably, not by the same magnitude.
...
country_stats <- function(code_, who_, by_affil = TRUE)
{
  ...
  return(paste0(pct(tbl[2, 2], sum(tbl[2,], 2)), "%", report_stat(chisq.test(tbl), p_option = "stars_only")))
}
...
Half or so of the US-based CS researchers (and in our dataset) are likely foreign-born \cite{frachtenberg20:survey,national20:science}, but this distinction does not appear to explain differences in the gender gap \cite{frachtenberg20:survey,goyette99:intersection,hango13:gender,tong10:place}.

One possible explanation is that as the host of most systems conferences in our dataset, the US might be more appealing to researchers who prefer domestic travel, such as parents of young children.
...
However, we found no evidence of a gender difference in this preference---not in the US, where there are actually fewer women in US-hosted conferences---and not more generally, since the correlation between a country's FAR by affiliation and by hosted conference is nonexistent
...
With few exceptions, primarily near southeast Asia, most individual countries also exhibit a higher FPR than FAR, which may be related to the subjective selection process of PC roles.
