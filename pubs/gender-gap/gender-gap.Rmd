...
role_genders <- data.frame(Role = c("Author", "Lead author", "Keynote speaker", "Panelist", "PC chair", "PC member", "Session chair"),
                  "Total" = repeated$N,
                  "Women" = paste0(repeated$pct_w, "%"),
                  "Unique" = unique$N,
...
The conferences we selected include some of the most prestigious systems conferences (based on indirect measurements such as Google Scholar's metrics), as well as several smaller or less-competitive conferences for contrast (Table \@ref(tab:sys-confs)).[^It should be noted that venue prestige and gender-gap are not independent. For example, men tend to self-cite more than women \cite{king17:men} and novel research published by women is less likely to be impactful \cite{hofstra20:diversity}. We do not address this gap within the scope of this paper, but note that we found no direct correlations between a conference prestige metrics and its ratio of female authors.]
...
knitr::kable(role_genders[c(1,2,5,6,3,4,7),], align=c("lrrrr"), booktabs = T, row.names = F,
             caption = "Researcher count and ratio of women by conference role for people with assigned genders. Researchers are either aggregated with repeats, counting multiple appearances in role, or identified uniquely per role. Roles include all authors, lead (first) authors only, program committee (PC) chairs and members, and conference presentation roles.")
```
...
But it is nevertheless consistent with past observations that women continue to senior academic ranks at a lower rate than men \cite{elsevier20:journey,fox06:engineering,frantzana19:women,gerhard07:undergraduate,mattis07:upstream}.
