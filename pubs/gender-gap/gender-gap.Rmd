...
abstract: |
 | The gender gap in computer science (CS) research is a well-studied problem, with an estimated ratio of 20%--30% of women researchers.  However, far less is known about gender representation in specific fields within CS. Here, we investigate the gender gap in one large field, computer systems. To this end, we combined data from 56 leading systems conferences with external demographic and bibliometric data to evaluate the ratio of women authors and the factors that might affect this ratio.
 |  Our main findings are that women represent only about 10% of systems researchers, and that this ratio is not associated with various conference factors such as size, prestige, double-blind reviewing, and inclusivity policies.  Author Research experience also does not significantly affect this ratio, but it does vary significantly by country and work sector.
...
Numerous studies estimate that only about 20%--30% of the CS research community are women \cite{cohoon11:cspapers,holman18:gender,national20:science,wang21:trends,way16:gender,zweben18:taulbee}.
...
To address this gap, we manually curated gender data from a large and representative cross section of the field.
...
This approach has been previously tested in numerous researcher populations, typically using automated gender inference from given names \cite{elsevier17:gender,lariviere13:bibliometrics,mattauch20:bibliometric}.
Because our methodology relies primarily on manually curated data, it has better coverage and accuracy than studies based on automated gender-inference approaches.

In addition to computing gender ratios, we also collected and analyzed conference statistics, demographic data, and bibliometric from Google Scholar and Semantic Scholar to examine how these factors interact with women researcher ratios.
...
The primary goal of this study was to quantify as precisely as possible the ratio of women in systems, which to the best of our knowledge, had never been done for the entire field. We found this ratio to be particularly low, at about 10%.
Our secondary goal was to look at a multiple of potential explanatory variables to evaluate which ones may have en effect on this gap. Notably, we found that implementing specific diversity policies was not associated with improved representation for women.
...
tmp <- sys_confs %>%
  ...
  dplyr::select(Conference, Date, Papers, Authors, Acceptance, Country) %>%
  arrange(Conference)
...
In total, our sample includes `r sum(sys_confs$npapers)` peer-reviewed papers.
...
`r round(mean(filter(sys_authors, gender == "F")$as_author, na.rm = T), 2)`
papers in our dataset on average, compared to mens'
`r round(mean(filter(sys_authors, gender == "M")$as_author, na.rm = T), 2)`
(`r report_test(t.test(filter(sys_authors, gender == "F")$as_author, filter(sys_authors, gender == "M")$as_author), 2, show_df = T)`).
...
For example, some studies have demonstrated gender homophily between reviewers and authors, leading to higher FARs when more of the reviewers are women \cite{helmer17:gender,murray19:gender}.
...
Of our `r nrow(sys_confs)` conferences, `r sum(sys_confs$diversity_effort)` implemented at least one of these proposals (Table \@ref(tab:diversity-policy)), but that did not ostensibly lead to higher FARs
...
Inclusivity initiatives may only represent one of the selection criteria when choosing a conference to publish in, and other criteria such as conference date, location, prestige, and subfield take precedence.
...
To summarize, inviting more women to visible conference roles and implementing diversity-focused policies likely contributes to more inclusive conferences \cite{campbell18:defence, ISC19:report}, but is insufficient on its own to spontaneously add women authors to the field.
...
### Author statistics per conference {-}
...
### Program committee statistics {-}
  ...
  * Mean reviewer load (approximate papers per reviewing day):  `r report_test(cor.test(conf_gender$author, conf_gender$mean_review_load, use = "pairwise.complete.obs"))`
...
<!--
The last result shows the largest correlation, albeit still very small.This correlation is unlikely to reveal a causal relationship, i.e., that inviting more women to the PC necessarily leads to increased FAR. As we have seen before, there is no real correlation between the two, but since conferences generally exhibit higher FPRs than FARs, it makes sense that conferences with higher PC participation in the authorship would also exhibit higher relative FARs.
-->
