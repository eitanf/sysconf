...
We also looked up each author's affiliation institute on GS to find their country of residence and work sector whenever they could be clearly inferred using hand-coded regular expressions.
...
Although our comparison data is necessarily constrained by the scalability of our manual collection approach, it still includes `r fmt(nrow(nonsys_authors))` non-unique authors from `r fmt(length(nonsys_confs))` of the top-cited non-systems CS conferences, based on GS metrics.
...
To round up our exploratory data analysis analysis, we computed a logistic-regression mixed-effects model to surface the factors most strongly associated with gender.
...
One important factor that was associated with gender differences in publication rate and impact was the possible role of resource requirements [@head13:funding].
...
Another factor in choice of fields is pay and prestige.
