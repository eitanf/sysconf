...
cleaning_imputation <- function(dat, ratio = THRESH_RATIO, s2_limit = S2_THRESH)
{
  dat <- dat %>%
    ...
    mutate(npubs_all_good = na_if(npubs/s2npubs < ratio | s2npubs/npubs < ratio, TRUE))
...
The conferences we selected include some of the most prestigious systems conferences (based on indirect measurements such as Google Scholar's metrics), as well as several smaller or less-competitive conferences for contrast (Table \@ref(tab:sys-confs), with additional details in the S2 Appendix).
...
Obviously, not all systems papers from 2017 are included in our set, and some papers that are in our set may not necessarily be considered part of systems (for example, if they lean more towards algorithms or theory).
...
All analyses were conducted in the statistical software package `R` [@rteam13:r].
The specific code for this study can be found in the file
...
metrics.summary <- ppl %>%
  ...
  mutate(Metric = recode_factor(Metric,
                                npubs = "Publications (GS)", hindex = "h-index",
                                citedby = "Citations (GS)",
                                s2npubs = "Publications (S2)",
...
rep.all <- rep.all %>%
  mutate(t_stat.t = as.numeric(t_stat.t),
         t_pval = as.numeric(t_pval),
         w_stat.W = as.numeric(w_stat.W),
         w_pval = as.numeric(w_pval),
         diff = as.numeric(median_sb) - as.numeric(median_db)) %>%
  rowwise() %>%
  mutate(t_label = paste0(round(t_stat.t, 2), format_p_value(t_pval, p_option = "stars")),
         w_label = paste0(round(diff, 2), format_p_value(w_pval, p_option = "stars")))
...
reg.all <- reg.all %>%
  mutate(linear_c = as.numeric(linear_c),
         linear_p = as.numeric(linear_p),
         logistic_c = as.numeric(logistic_c),
         logistic_p = as.numeric(logistic_p),
         linear_m_c = as.numeric(linear_m_c),
         linear_m_p = as.numeric(linear_m_p),
         logistic_m_c = as.numeric(logistic_m_c),
         logistic_m_p = as.numeric(logistic_m_p)
  ) %>%
  rowwise() %>%
  mutate(label_linear_c = paste0(round(linear_c, 2), format_p_value(linear_p, p_option = "stars")),
         label_linear_m = paste0(round(linear_m_c, 2), format_p_value(linear_m_p, p_option = "stars")),
         label_logistic_c = paste0(round(logistic_c, 2), format_p_value(logistic_p, p_option = "stars")),
         label_logistic_m = paste0(round(logistic_m_c, 2), format_p_value(logistic_m_p, p_option = "stars")))
...
reg.all %>%
  ...
  ggplot(aes(x = metric, y = predictor)) +
    ...
    scale_fill_gradient2(low = "orange", high = "blue") +
    geom_text(aes(label = label_linear_c)) +
...
reg.all %>%
  ...
  ggplot(aes(x = metric, y = predictor)) +
    ...
    scale_y_discrete(limits = rev) +
    scale_fill_gradient2(low = "orange", high = "blue") +
    geom_text(aes(label = label_linear_m)) +
    ylab("Additional predictor variable") +
    xlab("Reputation metric (max per paper)") +
    labs(fill = "DB coefficient") +
    theme_minimal() +
...
reg.all %>%
  ...
  ggplot(aes(x = metric, y = predictor)) +
    ...
    scale_y_discrete(limits = rev) +
    scale_fill_gradient2(low = "orange", high = "blue") +
    geom_text(aes(label = label_logistic_c)) +
    ylab("Additional predictor variable") +
...
reg.all %>%
  ...
  ggplot(aes(x = metric, y = predictor)) +
    ...
    scale_y_discrete(limits = rev) +
    scale_fill_gradient2(low = "orange", high = "blue") +
    geom_text(aes(label = label_logistic_m)) +
    ylab("Additional predictor variable") +
    xlab("Reputation metric in top 10% of authors") +
    labs(fill = "DB coefficient") +
    theme_minimal() +
...
# ggsave("Fig_11.eps", dpi = 600)
```



Figures \@ref(fig:logistic-regression) and \@ref(fig:logistic-mixed-regression) depict the effects of this binary treatment of author prestige on the association with reviewing policy, corrected for conference prestige.
...
dat <- rep.all %>%
  select(imp_type, aggregation, normalized, diff, w_pval, metric_type, w_label) %>%
  rename(stat_norm = normalized, stat = diff, pval = w_pval, label = w_label) %>%
  mutate(test = "Wilcoxon Signed Rank Test") %>%
  bind_rows(select(rep.all,
                   imp_type, aggregation, t_stat.t, t_pval, metric_type, t_label) %>%
              rename(stat = t_stat.t, pval = t_pval, label = t_label) %>%
...
# ggsave("Fig_14.eps", dpi = 600)
```


Fig. \@ref(fig:npubs-vs-hindex) compares these metrics across all authors with GS data, with the color of each point representing the percentage of papers in our set that each person published in SB or DB conferences.
