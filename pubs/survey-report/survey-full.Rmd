...
We can check for response bias in industry or government responses, by looking at another data source: the affiliation of an author's email address (or their Google Scholar profile if we couldn't find an email address in the paper). Of our `r nrow(authors)` unique authors, `r nrow(good_affils)` had an identifiable affiliation. Of these, `r pct(sum(good_affils$sector == "COM"), nrow(good_affils))`% had an industry affiliation, compared to `r pct(nrow(filter(demographics, position == "Industry Researcher")), nrow(good_positions))`% of the non-NA survey respondents (`r report_stat(chisq.test(com_table))`).
The difference for government researchers is a little larger --- `r pct(sum(good_affils$sector == "GOV"), nrow(good_affils))`% among survey respondents vs. `r pct(nrow(filter(demographics, position == "Government Researcher")), nrow(good_positions))`% in author emails (`r report_stat(chisq.test(gov_table))`), but still not significant enough to reject the null hypothesis of independence.
...
These two proportions are very close (`r report_stat(chisq.test(gender_table))`), leading us to believe there is no significant selection bias in respondent gender.
...
month_data <- survey %>%
  ...
  pivot_longer(-paper_id, names_to = "Author", values_to = "Months")

month_data %>%
  ggplot(aes(x = Months, fill = Author)) +
         geom_histogram(stat = "count", position = "dodge") + 
         scale_x_discrete(limits = c("1-3", "4-6", "7-9", "10-12", ">12", NA)) +
         xlab("months of research") +
         scale_fill_discrete(name = "author group")

month_table <- data.frame(A = c(nrow(filter(month_data, Author == "A", Months == "1-3")), nrow(filter(month_data, Author == "A", Months == "4-6")), nrow(filter(month_data, Author == "A", Months == "7-9")), nrow(filter(month_data, Author == "A", Months == "10-12")), nrow(filter(month_data, Months == "A", Months == ">12"))),
                          B = c(nrow(filter(month_data, Author == "B", Months == "1-3")), nrow(filter(month_data, Months == "B", Months == "4-6")), nrow(filter(month_data, Author == "B", Months == "7-9")), nrow(filter(month_data, Author == "B", Months == "10-12")), nrow(filter(month_data, Author == "B", Months == ">12"))))
...
The distributions are very similar overall (`r report_stat(chisq.test(month_table), 2)`), so we'll arbitrarily stick with the first non-NA response per paper in our future analyses.
...
(`r report_stat(chisq.test(table(leads$sector, leads$mor)))`).
Gender also didn't seem to play a role in duration of research (not depicted,
`r report_stat(chisq.test(table(leads$gender, leads$mor)[-3,]))`).
...
survey %>%
  ...
  pivot_longer(-paper_id, names_to = "Author", values_to = "Prior") %>%
  ggplot(aes(x = Prior, fill = Author)) +
         geom_histogram(stat = "count", position = "dodge") + 
...
survey %>%
  ...
  pivot_longer(-paper_id, names_to = "Author", values_to = "Native") %>%
  ggplot(aes(x = Native, fill = Author)) +
         geom_histogram(stat = "count", position = "dodge") + 
...
survey %>%
  ...
  pivot_longer(-paper_id, names_to = "Author", values_to = "Rebut") %>%
  ggplot(aes(x = Rebut, fill = Author)) +
         geom_histogram(stat = "count", position = "dodge") + 
         scale_x_discrete(limits = c(NA, "TRUE", "FALSE"), labels = c("NA", "Yes", "No")) +
...
(`r report_stat(chisq.test(data.frame(c(profs_yes, profs_no), c(nrow(yes_rebut_help) - profs_yes, nrow(no_rebut_help) - profs_no))), 4)`).
...
`r report_stat(chisq.test(data.frame(c(students_yes, students_no), c(nrow(yes_rebut_help) - students_yes, nrow(no_rebut_help) - students_no))), 4)`),
...
`r report_stat(t.test(no$npubs, yes$npubs))`).
We've also collected information on which authors serve on program committees (PCs) in any of our conferences, as another measure of experience. It agrees with the previous metrics: those authors satisified with the rebuttal process serve on an average of `r round(mean(yes$as_pc), 1)` PCs, compared to `r round(mean(no$as_pc), 1)` PCs for those who weren't (`r report_stat(t.test(no$as_pc, yes$as_pc))`). This is consistent with the mixed opinions we got directly from PC chairs on the question of rebuttals.
...
(`r report_stat(t.test(filter(survey, rebuttal_helpful == T)$gr_present, filter(survey, rebuttal_helpful == F)$gr_present), 3)`),
...
(`r report_stat(chisq.test(data.frame(c(women_yes, women_no), c(men_yes, men_no))))`).
...
(`r report_stat(chisq.test(data.frame(yes = summary(filter(survey, rebuttal_helpful == T)$understanding), no = summary(filter(survey, rebuttal_helpful == F)$understanding))))`).
...
(`r report_stat(t.test(no_rebut_help$prior_subs, yes_rebut_help$prior_subs))`).
...
(`r report_stat(t.test(filter(confs_with_subs, rebuttal == F)$prior_subs, filter(confs_with_subs, rebuttal == T)$prior_subs))`).
...
(`r report_stat(t.test(filter(all_confs, rebuttal == F, !is.na(acceptance_rate))$acceptance_rate, filter(all_confs, rebuttal == T)$acceptance_rate))`).
...
```{r why-not-helpful, echo=F, warning=F}
...
(`r report_stat(cor.test(all_confs$submissions, all_confs$pc_size, use = "pairw"))`),
but not necessarily more days to review
(`r report_stat(cor.test(all_confs$submissions, all_confs$review_days, use = "pairw"))`)
or shorter papers
(`r report_stat(cor.test(all_confs$submissions, -all_confs$mean_pages, use = "pairw"))`).
...
survey %>%
  ...
  pivot_longer(-paper_id, names_to = "Author", values_to = "RPP") %>%
  ggplot(aes(x = RPP, fill = Author)) +
         geom_histogram(stat = "count", position = "dodge") + 
...
(`r tmp <- review_counts %>% mutate(conference = gsub("_\\d\\d\\d", "", paper_id)) %>% left_join(all_confs); report_stat(cor.test(tmp$reviews, tmp$mean_review_load))`).
...
(`r report_stat(chisq.test(table(survey$mapped_gender, survey$understanding)))`),
especially in double-blind reviews, but not clearly so in single-blind reviews
(`r report_stat(chisq.test(table(filter(survey_with_confs, double_blind == F)$mapped_gender, filter(survey_with_confs, double_blind == F)$understanding)))`).
...
(`r report_stat(t.test(poor_understanding$prior_subs, good_understanding$prior_subs))`).
...
(`r report_stat(chisq.test(table(survey$understanding, survey$length)))`). But longer is not always better or necessary, as these short reviews still comprise
...
(`r report_stat(chisq.test(t(table(survey$native_english, survey$understanding))))`),
...
(`r report_stat(t.test(survey_with_pct_english$pct_english, english_pcs$pct_english))`),
...
(`r report_stat(chisq.test(data.frame(c(nrow(filter(poor_understanding, rebuttal == T)), nrow(filter(survey_with_confs, rebuttal == T))), c(nrow(filter(poor_understanding, rebuttal == F)), nrow(filter(survey_with_confs, rebuttal == F))))))`),
the conference's mean historical citations 
(`r report_stat(t.test(poor_understanding$mean_historical_citations, good_understanding$mean_historical_citations))`),
or H5 index
(`r report_stat(t.test(poor_understanding$h5_index, good_understanding$h5_index))`).
...
(`r report_stat(chisq.test(table(survey$helpfulness, survey$length)))`).
...
(`r report_stat(chisq.test(table(survey$helpfulness, survey$understanding)))`).
...
(`r report_stat(chisq.test(table(survey$mapped_gender, survey$helpfulness)))`),
especially in double-blind conferences
(`r report_stat(chisq.test(table(filter(survey_with_confs, double_blind == T)$mapped_gender, filter(survey_with_confs, double_blind == T)$helpfulness)))`),
and non-native speakers judge reviews more helpful, as well as better understood, compared to native speakers.
(`r report_stat(chisq.test(t(table(survey$native_english, survey$helpfulness))))`).
...
(`r report_stat(chisq.test(table(survey$fairness, survey$understanding)))`)
and helpfulness
(`r report_stat(chisq.test(table(survey$fairness, survey$helpfulness)))`),
which in turn implies a strong association with review length, 
(`r report_stat(chisq.test(t(table(survey$length, survey$fairness))))`),
gender in double-blind reviews
(`r report_stat(chisq.test(table(filter(survey_with_confs, double_blind == T)$mapped_gender, filter(survey_with_confs, double_blind == T)$fairness)))`),
and English level.
(`r report_stat(chisq.test(t(table(survey$native_english, survey$fairness))))`).
...
(`r lead_grades = left_join(filter(survey, !is.na(mapped_gender)), filter(roles, role == "lead_author"), by = c("paper_id" = "key")); report_stat(t.test(filter(lead_grades, mapped_gender == "F")$gr_overall, filter(lead_grades, mapped_gender == "M")$gr_overall))`),
