...
We can check for response bias in industry or government responses, by looking at another data source: the affiliation of an author's email address (or their Google Scholar profile if we couldn't find an email address in the paper). Of our `r nrow(authors)` unique authors, `r nrow(good_affils)` had an identifiable affiliation. Of these, `r pct(sum(good_affils$sector == "COM"), nrow(good_affils))`% had an industry affiliation, compared to `r pct(nrow(filter(demographics, position == "Industry Researcher")), nrow(good_positions))`% of the non-NA survey respondents (`r report_test(chisq.test(com_table))`).
The difference for government researchers is a little larger --- `r pct(sum(good_affils$sector == "GOV"), nrow(good_affils))`% among survey respondents vs. `r pct(nrow(filter(demographics, position == "Government Researcher")), nrow(good_positions))`% in author emails (`r report_test(chisq.test(gov_table))`), but still not significant enough to reject the null hypothesis of independence.
...
These two proportions are very close (`r report_test(chisq.test(gender_table))`), leading us to believe there is no significant selection bias in respondent gender.
...
The distributions are very similar overall (`r report_test(chisq.test(month_table), 2)`), so we'll arbitrarily stick with the first non-NA response per paper in our future analyses.
...
(`r report_test(chisq.test(table(leads$sector, leads$mor)))`).
Gender also didn't seem to play a role in duration of research (not depicted,
`r report_test(chisq.test(table(leads$gender, leads$mor)[-3,]))`).
...
(`r report_test(chisq.test(data.frame(c(profs_yes, profs_no), c(nrow(yes_rebut_help) - profs_yes, nrow(no_rebut_help) - profs_no))), 4)`).
...
`r report_test(chisq.test(data.frame(c(students_yes, students_no), c(nrow(yes_rebut_help) - students_yes, nrow(no_rebut_help) - students_no))), 4)`),
...
`r report_test(t.test(no$npubs, yes$npubs))`).
We've also collected information on which authors serve on program committees (PCs) in any of our conferences, as another measure of experience. It agrees with the previous metrics: those authors satisified with the rebuttal process serve on an average of `r round(mean(yes$as_pc), 1)` PCs, compared to `r round(mean(no$as_pc), 1)` PCs for those who weren't (`r report_test(t.test(no$as_pc, yes$as_pc))`). This is consistent with the mixed opinions we got directly from PC chairs on the question of rebuttals.
...
(`r report_test(t.test(filter(survey, rebuttal_helpful == T)$gr_present, filter(survey, rebuttal_helpful == F)$gr_present), 3)`),
...
(`r report_test(chisq.test(data.frame(c(women_yes, women_no), c(men_yes, men_no))))`).
...
(`r report_test(chisq.test(data.frame(yes = summary(filter(survey, rebuttal_helpful == T)$understanding), no = summary(filter(survey, rebuttal_helpful == F)$understanding))))`).
...
(`r report_test(t.test(no_rebut_help$prior_subs, yes_rebut_help$prior_subs))`).
...
(`r report_test(t.test(filter(confs_with_subs, rebuttal == F)$prior_subs, filter(confs_with_subs, rebuttal == T)$prior_subs))`).
...
(`r report_test(t.test(filter(all_confs, rebuttal == F, !is.na(acceptance_rate))$acceptance_rate, filter(all_confs, rebuttal == T)$acceptance_rate))`).
...
(`r report_test(cor.test(all_confs$submissions, all_confs$pc_size, use = "pairw"))`),
but not necessarily more days to review
(`r report_test(cor.test(all_confs$submissions, all_confs$review_days, use = "pairw"))`)
or shorter papers
(`r report_test(cor.test(all_confs$submissions, -all_confs$mean_pages, use = "pairw"))`).
...
(`r tmp <- review_counts %>% mutate(conference = gsub("_\\d\\d\\d", "", paper_id)) %>% left_join(all_confs); report_test(cor.test(tmp$reviews, tmp$mean_review_load))`).
...
(`r report_test(chisq.test(table(survey$mapped_gender, survey$understanding)))`),
especially in double-blind reviews, but not clearly so in single-blind reviews
(`r report_test(chisq.test(table(filter(survey_with_confs, double_blind == F)$mapped_gender, filter(survey_with_confs, double_blind == F)$understanding)))`).
...
(`r report_test(t.test(poor_understanding$prior_subs, good_understanding$prior_subs))`).
...
(`r report_test(chisq.test(table(survey$understanding, survey$length)))`). But longer is not always better or necessary, as these short reviews still comprise
...
(`r report_test(t.test(survey_with_pct_english$pct_english, english_pcs$pct_english))`),
...
(`r report_test(chisq.test(data.frame(c(nrow(filter(poor_understanding, rebuttal == T)), nrow(filter(survey_with_confs, rebuttal == T))), c(nrow(filter(poor_understanding, rebuttal == F)), nrow(filter(survey_with_confs, rebuttal == F))))))`),
the conference's mean historical citations 
(`r report_test(t.test(poor_understanding$mean_historical_citations, good_understanding$mean_historical_citations))`),
or H5 index
(`r report_test(t.test(poor_understanding$h5_index, good_understanding$h5_index))`).
...
(`r report_test(chisq.test(table(survey$helpfulness, survey$length)))`).
...
(`r report_test(chisq.test(table(survey$helpfulness, survey$understanding)))`).
...
(`r report_test(chisq.test(table(survey$mapped_gender, survey$helpfulness)))`),
especially in double-blind conferences
(`r report_test(chisq.test(table(filter(survey_with_confs, double_blind == T)$mapped_gender, filter(survey_with_confs, double_blind == T)$helpfulness)))`),
and non-native speakers judge reviews more helpful, as well as better understood, compared to native speakers.
(`r report_test(chisq.test(t(table(survey$native_english, survey$helpfulness))))`).
...
(`r report_test(chisq.test(table(survey$fairness, survey$understanding)))`)
and helpfulness
(`r report_test(chisq.test(table(survey$fairness, survey$helpfulness)))`),
which in turn implies a strong association with review length, 
(`r report_test(chisq.test(t(table(survey$length, survey$fairness))))`),
gender in double-blind reviews
(`r report_test(chisq.test(table(filter(survey_with_confs, double_blind == T)$mapped_gender, filter(survey_with_confs, double_blind == T)$fairness)))`),
and English level.
(`r report_test(chisq.test(t(table(survey$native_english, survey$fairness))))`).
...
(`r lead_grades = left_join(filter(survey, !is.na(mapped_gender)), filter(roles, role == "lead_author"), by = c("paper_id" = "key")); report_test(t.test(filter(lead_grades, mapped_gender == "F")$gr_overall, filter(lead_grades, mapped_gender == "M")$gr_overall))`),
