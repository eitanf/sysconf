---
title: "State of the Stats:  Representation of Women in HPC Conferences"
affiliation:
  ...
  institution:
    ...
    - name: Reed College
      ...
      author:
        - name: Rhody Kaner
abstract: |
  Women are severely under-represented in the HPC workforce. Following the aphorism "if you can't measure it, you can't improve it," this paper aims to provide tangible and reproducible data on this gender gap. Specifically, this paper provides statistics on women's representation in HPC conferences, focusing mainly on authors of peer-reviewed papers, who serve as the keystone for future advances in the field.

  To this end, we analyzed participant data for nine HPC and HPC-related academic conferences from a single year. In addition to examining gender distributions, we looked at other demographic factors, such as the authors' country and sector, and at post-publication statistics of the papers.

  Our main finding are that women represent only about 10% of all HPC paper authors, with some geographical variations. Women are even more under-represented among industry researchers and at higher experience levels.
...
Variance in gender distribution by field of study is an active area of research [@cohoon06:critical; @cohoon11:cspapers], but some subfields remain poorly examined [@fox06:engineering]. One of these areas is High-Performance Computing (HPC), a large and vibrant research field with significant impact on the economy and the sciences, including computer science (CS). Despite the enormous significance of the field, less than 17% of its workforce is estimated to be women, although the exact number is difficult to ascertain [@trader16:whpc]. The focus of this paper is to reduce some of this uncertainty by computing reproducible metrics, specifically on one important population of the HPC community, namely, HPC researchers. Although this paper cannot fully address the complex question of why women are so severely under-represented in HPC, it attempts to quantify this under-representation, with two goals in mind: to better understand its causes, and to establish a baseline against which to measure the effects of future efforts and policies to increase gender diversity.


# Methodology and Data {#sec:methodology}

As in most subfields of computer science, the primary channel for publishing research results in HPC is peer-reviewed conferences [@franceschet10:role; @freyne10:relative; @patterson99:evaluating; @vardi09:conferences]. For this study we collected extensive data from nine HPC-related conferences that took place in 2017 (Table \@ref(tab:hpc-confs)).
...
The largest of these conferences, [SC](http://supercomputing.org/) takes place in the US every November and attracts many thousands of attendees. SC is also notable for its diversity initiatives, including an inclusivity chair, on-site childcare, a code of conduct, and diversity-related content. The other flagship HPC conference, [ISC](https://www.isc-hpc.com/) takes place in Germany every summer and also has a diversity chair. The other seven conferences are not exclusive to HPC, but contain numerous HPC papers. None of them displayed diversity initiatives on their website for 2017.
...
The most crucial piece of our data is, of course, gender information. Given the full name and affiliation of most researchers, we searched online for each person's pronouns or picture to infer their gender.

This approach has several limitations. First, gender identity is complex and non-binary, and yet as in many similar studies, we are confined to representing gender as a simple binary choice because of the available data [@cohoon11:cspapers; @elsevier17:gender; @fox16:gender; @holman18:gender; @huang19:historical; @karimi16:gender]. Second, not all researchers presented unambiguous Web presence or gender. For those
...
we marked their gender as NA and omitted them from most analyses, which may introduce a small bias.
...
For reproducibility, all of the data and source code files for this paper will be posted online upon publication.
...
We analyzed the conferences in our set through the lens of the ratio of women in different roles across all conference papers (Fig. \@ref(fig:female-ratios)).
...
We start by looking at the ratio of female authors and lead (first) authors across all conference papers. Since these roles are peer-reviewed and ideally selected from the overall author population based on the merit of their papers alone, this sample population may be the most representative of the overall female population across publishing HPC researchers.
The result, if indeed indicative of the overall population, is alarming. Only about
...
Most conferences only have one or two PC chairs, so it is difficult to draw statistical conclusions from this role. Nevertheless, it is instructive to note that four of the nine conferences were chaired by no woman whatsoever.
...
are women, about twice the rate among authors. Even without the larger SC, the ratio is still 
...
Finally, we look at two more conference roles: keynote speakers and session chairs. These roles are only collateral to the main technical content of a conference. But they can be very visible to conference attendees, and as such, can represent the "face" of the conference. A conference with low female visibility in these roles could conceivably deter further women from joining or staying in the conference or field. Like PC chairs, keynote speakers are few, but again we see four conferences with no women. Perhaps worse still, three conferences had no female session chairs whatsoever, out of a total of
`r nrow(filter(hpc_people, conf %in% c("HPDC", "HPCC", "HiPC"), role == "Session chair"))`
session chairs for these three!
...
Not all papers in our conference set relate directly to HPC. To limit our discussion only to HPC papers, as opposed to HPC conferences, we skimmed each paper and tagged it as "HPC" if its topic related directly to high performance hardware or software.^[The determination of this tag was based on our experience in HPC research, and is therefore subjective and error-prone.]
...
authors of HPC papers with known gender,
...
If we restrict our perspective to lead authors, the picture is similar: of the
...
a statistically insignificant difference from the previous ratio
...
hpc_people %>%
  ...
  left_join(filter(citations, near(months, 24, 2)))
...
One way to measure a paper's influence is through its citations over time. We have allowed our data set to age to the point where all papers could be discovered and cited by other researchers. The density plot in Fig. \@ref(fig:citations) shows the distributions of numbers of citations of the papers 24 months after publication, broken up by the gender of the lead author and the conference.
...
papers authored by men. The mean citations number is roughly the same for both genders. There is one outlier at the very long tail of the distribution. This paper, lead by a female author, garnered more than 100 citations, but does not appear to be strongly related to HPC [@liang17:provchain]. When we exclude this paper, the mean citations for women's papers is somewhat lower than men's
(`r fc <- filter(cites, gender == "F", citations < 100)$citations; fc %>% mean() %>% round(2)` vs.
`r mc <- filter(cites, gender == "M", citations < 100)$citations;  mc %>% mean() %>% round(2)`,
`r report_t(t.test(fc, mc), 2)`).
More pronounced is the difference between the percentage of papers with 5 citations or higher, 
`r pct(sum(mc >= 5), length(mc))`% for men vs. `r pct(sum(fc >= 5), length(fc))`% for women.
...
Unsurprisingly, most of these researchers hail from the West, although China, Japan, and India are also prominent (Table \@ref(tab:top-countries)). The United States has the highest percentage of women of any country with more than 15 researchers in this list, although it is still a far cry from gender equality. On the opposite end, Japan stands out among developed countries with particularly low female representation.
...
tbl <- countryd %>%
  group_by(country.y) %>%
  summarize(total = n(), pct_w = round(100 * sum(gender == "F", na.rm = T) / sum(!is.na(gender)), 2)) %>%
  arrange(desc(total)) %>%
  head(10)
names(tbl) <- c("Country", "Total", "% Women")
knitr::kable(tbl, booktabs = T, align = c("l", "r", "r"), caption = "Top ten countries by number of researchers")
```

Another way to break down these numbers is by geographic region and conference role (Table \@ref(tab:region)). This perspective confirms the stark lack of geographic diversity in the field, with a full half of the authors
(`r pct(nrow(filter(countryd, role=="Author", country == "US")), nrow(filter(countryd, role=="Author")), 2)`%)
associated with US email addresses, and another 
`r pct(nrow(filter(countryd, role=="Author", subregion == "Western Europe")), nrow(filter(countryd, role=="Author")), 2)`%
from Western Europe.
It is encouraging to note, however, that Western reviewers are not over-represented compared to authors, as has been observed in journals of other fields [@publons18:peer]. For example, the percentage of PC members from the US or Western Europe are fairly similar to the authors'
...
In terms of representation of women, none of the larger regions deviates much from the $\approx{10\%}$ overall percentage. The regions with fewer than 25 authors or so exhibit more variance, due to the small denominators (for example, a single female author from Eastern Europe would have bumped the percentage from zero to $\approx{8\%}$).
...
Another statistic we can evaluate from this data set is the previous research experience of researchers. We can approximate this experience by identifying the unique GS profile of researchers when possible (in our dataset,
`r pct(nrow(filter(demo, !is.na(hindex))), nrow(demo), 2)`
had a uniquely identifiable GS profile). The profile contains various bibliometric measures that can approximate the experience and research influence of a person. For example, Fig. \@ref(fig:npubs) depicts the distribution of one such metric, the total number of previous publications (circa their publication's date).
...
As expected, PC members generally have more experience (publications) than authors, especially among women, suggesting that perhaps more of the female authors are novices, in relative terms. Another interesting observation is that the males' distribution "pulls to the right" (note the long-tailed distribution). In other words, there appear to be relatively more men in more experienced or senior positions. This disparity may be attributed to the observation from other fields that women do not continue to senior research positions in the same rate as men [@fox06:engineering; @gerhard07:undergraduate; @mattis07:upstream].

To confirm these observations, we stratified all our researchers into the following three groups, using the experience metric of H-index [@hirsch05:index]: those with an H-index of less than 12, those with an H-index of 13 to 18, and the rest. We conveniently named these groups novices, mid-career, and experienced, although these breaks are arbitrary [@hirsch05:index]. Indeed, Fig. \@ref(fig:seniority) shows that fewer women in HPC reach the senior ranks of research, especially among authors
...
This paper presented recent statistics on the representation of women in nine HPC conferences during 2017. The picture that emerges shows a field of research very far from achieving gender equality. Men in HPC comprise the vast majority of researchers ($\approx{90\%}$), occupy most visible conference roles, and exhibit on average more research experience and somewhat higher citation counts.

There are some indications that representation is improving, however. Both SC and ISC are actively measuring inclusivity metrics and have shown steady progress in female representation in 2018 and 2019. As the two conferences with a dedicated diversity chair and active push toward inclusion, they may serve as a benchmark to estimate the effect of diversity efforts. In this short term, the generality of these improvements may be limited, but is encouraging. At the very least, these efforts are designed to make conferences more hospitable to all attendees, which may result in a long term improvement in gender representation. We plan to follow up at regular frequency to evaluate this hypothesis.
...
I would like to thank Kelly Shaw for spearheading this research, as well as Josh Reiss for his part in the collection of gender data. This work was supported by a grant from the Social Justice Fund at Reed College.
-->
