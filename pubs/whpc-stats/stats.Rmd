...
(`r report_stat(chisq.test(data.frame(all_authors = c(nrow(filter(hpc_a, gender == "F")), nrow(filter(hpc_a, gender == "M"))), leads = c(nrow(filter(hpc_l, gender == "F")), nrow(filter(hpc_l, gender == "M"))))), 4)`).
...
(`r report_stat(chisq.test(data.frame(f = c(sum(fc >= thrs), sum(fc < thrs)), m = c(sum(mc >= thrs), sum(mc < thrs)))), 2)`).
