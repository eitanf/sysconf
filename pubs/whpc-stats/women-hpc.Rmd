...
The complete dataset and source code necessary to reproduce this analysis can be found in the artifact appendix, as well as at [https://github.com/eitanf/sysconf](https://github.com/eitanf/sysconf). The specific analyses of this article are in the file `pubs/gender-gap.Rmd`.
...
`r nrow(demo)`
authors and PC members in our dataset, spanning numerous countries and institutions. We can estimate the distribution of these demographics by looking at email addresses or GS affiliations. Using simple regular expressions, we mapped most domains to an institution's country and sector, and marked the rest as unknown. We also collected the various GS bibliometrics for each researcher to estimate their research experience, which is our next analysis.
...
```{r sector, echo=F, message=F, out.width='0.5\\textwidth', fig.height=3.5, fig.pos= "!", fig.cap="Representation of women by sector and role."}
demo %>%
  drop_na(gender) %>%
  drop_na(sector) %>%
