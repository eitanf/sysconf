...
In a recent study we found that the ratio of female authors in computer systems conferences is particularly low, even compared to the rest of CS [@yamamoto22:collab].
This large underrepresentation cannot be fully explained by review bias, differences in collaboration patterns, and numerous other demographic and conference factors [@frachtenberg22:gender-gap].
The statistically significant gender gap raises the question, why is the underrepresentation of women in computer systems authorship so high?
...
We may never be able to explain it fully; however, we can examine many of the factors that have been previously identified as possible contributors to the gender gap in CS and other sciences in general, in an attempt attempt to assess whether their effect is exacerbated for computer systems in particular.
...
Our dataset includes extensive information on the conferences, papers, and participants.
We manually matched most authors with a *perceived gender*, based on pronouns and photos, rather than using automated approaches using name-based inference, because their accuracy and recall can be quite low for some populations.
...
We present each factor as a hypothesis: does this factor have an increased effect on the gender gap in systems in particular?
The hypotheses are organized into two groups, six hypotheses where our dataset provide evidence for or against an increased effect, and four for which we have no specific data but can speculate based on the characteristics of the field.
For each factor, we briefly explain its observed effects on the gender gap from the literature, and then try to find support in the data (or lack thereof) for an increased effect of this factor in systems.
Specifically, since most of our detailed data is on systems papers, and not on other fields, we try to extract quantitative evidence for each factor by primarily looking at the effects if each factor within systems alone, unless other supporting data is available.
...
Paradoxically, the high gender gap in a field could itself by partially caused by the existing high gender gap.
The lack of female mentors, peers, and role models dissuades women from entering or remaining in a field where they can feel unsupported and singled-out [@cohoon06:critical; @frantzana19:women; @kahn17:women].
...
In the `r fmt(n_distinct(full_auths$key))` systems papers we looked at, when the senior (last) author is a woman, nonsenior authors include
...
This paucity of senior women suggests that junior women have about 50% better chance to find a senior woman to collaborate with outside of systems.
 

## H2: Highly collaborative fields are less welcoming to gender minorities {-}

Another consequence of having fewer peers and mentors is that finding collaborators for research project may be more difficult for women in certain experimental fields [@abbate12:recoding], especially those that typically require larger research teams to begin with [@yamamoto22:collab].
...
(Men also averaged 2% larger team sizes, but this difference was not statistically significant.)


## H3: Women are better represented in human-centered fields {-}

It has been shown that owing to traditional social roles, women tend to be more people-centered and communally oriented than men.
On the other hand, most science and technology careers are not perceived as focusing heavily on these values [@frantzana19:women; @kahn17:women; @beyer04:deterrents; @diekman15:new]. 
...
That perception alone may further deter women’s entry to the field.
...
In particular, work in computer systems is very similar to other engineering disciplines, with its emphasis on efficiency, trade-off management, and practical applications.
In fact, at least `r pct(218, nrow(papers), 0)`% of the systems papers in our dataset have at least one author affiliated with an academic engineering department, although the percentage of authors with an electrical or computer engineering degree is likely even higher.
...
As a field related to engineering, we might expect computer systems to exhibit similarly large gender gaps for industry researchers.
Moreover, since systems research and development is increasingly central to the success of both technology startups and large-scale companies such as Google, Amazon, and Microsoft, we might expect the high demand for skilled workforce in the industry to amplify this factor.
However, the hypothesis that an industry affiliation leads to a higher gender gap in research is only weakly supported by our data.

By extracting the email address associated with each author in our systems paper, we can roughly categorize each author with a uniquely identifiable affiliation as either in government, academia, or industry [@frachtenberg22:gender-gap].
Of the 
...
It is well-known that women in the sciences and CS leave academia at a higher rate than men (the so-called “leaky pipeline”) [ @fox06:engineering; @camp02:incredible; @kahn17:women].
...
`r pct (nrow(filter(gen_auths, gender=="F", field == "Computer systems")), nrow(filter(gen_auths, field == "Computer systems")))`%, compared to the ratios in nonsystems papers
...
Girls were also found to be less likely than boys to open up a computer [@beyer04:deterrents].
Since women are dissuaded from learning about hardware and software at a young age, they may be less adept than men with the low-level programming skills required in systems.
...
Some women may already feel that a CS career may be hard to reconcile with raising a family [@beyer04:deterrents], and the perception of computer systems careers as particularly demanding may exacerbate this problem.


---


With so much evidence pointing out the lack of support, mentorship, and peers among female systems researchers, it is little wonder that the first support group for women in computing, "systers", was created specifically for the systems field [@abbate12:recoding].
The possible explanations to the gender gap we listed here is by all means not exhaustive.
Other well-known factors we didn’t discuss include occupational stereotypes, ability, demographics, discrimination, and bias.
...
This short list also includes no suggestions for reducing this gender gap in systems.
Many of the studies that shed light on the systemic causes of the gap in CS and other science also made concrete suggestions for improvements, which may similarly apply well for the field of computer systems [@cohoon06:critical; @mattis07:upstream].
