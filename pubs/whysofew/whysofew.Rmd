...
field_summary <- gen_auths %>%
  group_by(field) %>%
  summarize(.groups = "keep", Authors = n(), FAR = sum(gender == "F") / n()) %>%
  ungroup()
...
It has been shown that due to traditional social roles, women tend to be more people-centered and communally oriented than men and most STEM related careers are not perceived as focusing heavily on these values [@diekman15:new; @kahn17:women]. The underrepresentation of gender minorities in computer systems and other centered fields of CS can be seen through the oversaturation of human-centered subfields of CS such as human-computer interaction and computer education. This is likely due to the strong emphasis on benefiting one’s community and directly impacting individuals. Computer systems is not seen in the same light as the communal values are not as noticeable.
...
One thought as to why there are so few women in systems is that they start publishing in systems and then decide to leave academia for some reason. [Sentence here about why we look at retention rate?]. In examining the number of women compared to the number of men who stopped publishing after contributing to a systems paper, surprisingly, women did not leave academia at a higher rate than their male counterparts. Notably, in 2019 males left the field of computer systems research at a slightly higher rate with 7.5% more males last publishing in 2019 as opposed to their female counterparts.
...
Several studies have found that competitive pressures seem to deter women from entering certain fields, including CS [@kahn17:women]. The design and implementation of computer systems particularly emphasizes various performance metrics, and often pits one system against another. It is possible that elements of this competitive culture deter researchers who are risk- and conflict-averse or would rather focus on designing new capabilities.
